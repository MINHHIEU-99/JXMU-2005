...
----------------------------------------------------
function main()
dofile("script/vng_event/eventpgaming/thang10/banhtrungthuthuong.lua")
-------------------------------------
...
return 1 end
	if (nDayNow >= nTime1) and (nDayNow <= nTime2) then
			if GetTask(BANHTRUNGTHUTHUONG) < nGioiHanEventThuong then
				if CalcFreeItemCellCount() >= 10 then
					tbAwardTemplet:GiveAwardByList(tbBanhTrungThuThuong,1)
					SetTask(BANHTRUNGTHUTHUONG,GetTask(BANHTRUNGTHUTHUONG)+1)
				else
					Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y sö dông",0) return 1
				end
			else
				Say("Mçi Nh©n VËt ChØ Sö Dông Tèi §a "..nGioiHanEventThuong.." B¸nh trung thu th­êng Trong Suèt Thêi Gian Ho¹t §éng") return 1
			end
	else
