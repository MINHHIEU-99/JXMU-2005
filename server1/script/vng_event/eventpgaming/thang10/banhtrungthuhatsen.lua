...
----------------------------------------------------
function main()
dofile("script/vng_event/eventpgaming/thang10/banhtrungthuhatsen.lua")
-------------------------------------
...
return 1 end
	if (nDayNow >= nTime1) and (nDayNow <= nTime2) then
			if GetTask(BANHTRUNGTHUHATSEN) < nGioiHanEventThuong then
				if CalcFreeItemCellCount() >= 10 then
					tbAwardTemplet:GiveAwardByList(tbBanhTrungThuHatSen,1)
					SetTask(BANHTRUNGTHUHATSEN,GetTask(BANHTRUNGTHUHATSEN)+1)
				else
					Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y sö dông",0) return 1
				end
			else
				Say("Mçi Nh©n VËt ChØ Sö Dông Tèi §a "..nGioiHanEventThuong.." B¸nh trung thu h¹t sen Trong Suèt Thêi Gian Ho¹t §éng") return 1
			end
	else
