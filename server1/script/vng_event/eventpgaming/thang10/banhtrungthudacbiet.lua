...
----------------------------------------------------
function main()
dofile("script/vng_event/eventpgaming/thang10/banhtrungthudacbiet.lua")
-------------------------------------
...
return 1 end
	if (nDayNow >= nTime1) and (nDayNow <= nTime2) then
			if GetTask(BANHTRUNGTHUDB) < nGioiHanEventThuong then
				if CalcFreeItemCellCount() >= 10 then
					tbAwardTemplet:GiveAwardByList(tbBanhTrungThuDB,1)
					SetTask(BANHTRUNGTHUDB,GetTask(BANHTRUNGTHUDB)+1)
				else
					Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y sö dông",0) return 1
				end
			else
				Say("Mçi Nh©n VËt ChØ Sö Dông Tèi §a "..nGioiHanEventThuong.." B¸nh trung thu ®Æc biÖt Trong Suèt Thêi Gian Ho¹t §éng") return 1
			end
	else
