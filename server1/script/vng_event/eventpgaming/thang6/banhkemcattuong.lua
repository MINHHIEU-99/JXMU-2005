...
----------------------------------------------------
function main()
dofile("script/vng_event/eventpgaming/thang6/banhkemcattuong.lua")
...
return 1 end
	if (nDayNow >= nTime1) and (nDayNow <= nTime2) then
			if GetTask(BANHKEMCATTUONG) < nGioiHanEventDacBiet then
				if CalcFreeItemCellCount() >= 10 then
					tbAwardTemplet:GiveAwardByList(tbBanhKemCatTuong,1)
					SetTask(BANHKEMCATTUONG,GetTask(BANHKEMCATTUONG)+1)
				else
					Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y sö dông",0) return 1
				end
			else
				Say("Mçi Nh©n VËt ChØ Sö Dông Tèi §a "..nGioiHanEventDacBiet.." B¸nh Kem C¸t T­êng Trong Suèt Thêi Gian Ho¹t §éng") return 1
			end
		else
