...
----------------------------------------------------
function main()
dofile("script/vng_event/eventpgaming/thang6/banhkemnhuy.lua")
-------------------------------------
	...
	if (nDayNow >= nTime1) and (nDayNow <= nTime2) then
			if GetTask(BANHKEMNHUY) < nGioiHanEventThuong then
				if CalcFreeItemCellCount() >= 10 then
					...
					SetTask(BANHKEMNHUY,GetTask(BANHKEMNHUY)+1)
				else
					Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y sö dông",0) return 1
				end
			else
				Say("Mçi Nh©n VËt ChØ Sö Dông Tèi §a "..nGioiHanEventThuong.." B¸nh Kem Nh­ ý Trong Suèt Thêi Gian Ho¹t §éng") return 1
			end
		else
