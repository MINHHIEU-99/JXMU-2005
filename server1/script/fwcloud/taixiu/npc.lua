...
local tab_Content = {
"Më b¸t (1.xu)./mobat",
"KiÓm tra kÕt qu¶ theo m· phiªn ®¸nh./dienmaphien",
"Tho¸t./no"
	}
Say("<color=cyan>Nhµ c¸i<color>\n<color=green>KÕt qu¶ tr­íc phiªn thø - <color=yellow>"..phientruoc3.."\n"..ketquatruoc3.."\n<color=green>KÕt qu¶ tr­íc phiªn thø - <color=yellow>"..phientruoc2.."\n"..ketquatruoc2.."\n<color=green>KÕt qu¶ tr­íc phiªn thø - <color=yellow>"..phientruoc.."\n"..ketquatruoc, getn(tab_Content), tab_Content);       
end;

function dienmaphien()
AskClientForNumber("checkketqua", 1, 10000, "H·y nhËp m· phiªn: ");
end

function checkketqua(maphien)
server_loadfile(tbKandy.szFile)
local ketquatruoc = server_getdata(tbKandy.szFile,"PHIEN_OLD_CHECK",maphien);
Say("KÕt qu¶ cña phiªn " .. maphien .. " lµ - "..ketquatruoc)
end

function mobat()
	local nCount_muaban = CalcEquiproomItemCount(4,417,1,-1)
	if  nCount_muaban < 1  then
	Say("B¹n kh«ng cã mang theo 1 xu.")
return
end
...
if nRan == 1 then
szTitle = "Phiªn " .. phienhientai .." lµ "
szTitle1 = "3 tr¾ng 1 ®en [ LÎ ] - C¸i : [" .. GetName() .."]" 
end
...
if nRan == 3 then
szTitle = "Phiªn " .. phienhientai .." lµ "
szTitle1 = "3 ®en 1 tr¾ng [ LÎ ] - C¸i : [" .. GetName() .."]" 
end
