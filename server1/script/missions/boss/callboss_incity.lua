...
QY_GOLDBOSS_DPOS_INFO = {
	-- [566]	= {"Cæ B¸ch­",		566, 	95,	342, 	1,"\\settings\\maps\\½­ÄÏÇø\\Î÷É½Óì\\hanmengboss.txt"},
	...
	-- [568]	= {"Hµ Linh Phiªu",		568,	95,	340,	4,"\\settings\\maps\\½­ÄÏÇø\\Î÷É½Óì\\big_goldboss.txt"},
	...
	-- [563]	= {"Gia LuËt TÞ Ly",		563, 	95,	341, 	4,"\\settings\\maps\\¶«±±Çø\\Ä®±±²ÝÔ­\\shansinanboss.txt"},
	-- [562]	= {"§¹o Thanh Ch©n Nh©n",			562, 	95,	181, 	1,"\\settings\\maps\\½­ÄÏÇø\\Á½Ë®¶´ÃÔ¹¬\\lanyiyiboss.txt"},
...
function QY_MakeBoss_RandInCity(bossid, bosslvl, series, mapid, posx, posy, bossname, str)
	...
	if (npcindex > 0) then
		...
		if (nHour == 19) then
			SetNpcParam(npcindex, 1, 1); --19:30 Boss XuÊt HiÖn
...
function CallBossDown_Outter(bossid, str)
	...
	if (nHour == 19 and npcindex > 0) then
		SetNpcParam(npcindex, 1, 1); --19:30 BOSS XUAT HIEN
		RemoteExecute("\\script\\mission\\boss\\bigboss.lua", "AddGoldBossCount", 0);
	end
end
...
function CallBossDown_Fixure()
	for i = 1, getn(QY_NORMALBOSS_INFO) do
		...
		if (nworldidx >= 0) then
			...
			SetNpcDeathScript(npcindex, "\\script\\missions\\boss\\smailbossdeath.lua");
