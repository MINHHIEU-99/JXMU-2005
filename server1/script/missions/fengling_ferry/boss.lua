...
nNpcIndex = AddNpc(1692, 85,SubWorldID2Idx(337), 1634*32, 3221*32, 1, "Thñy TÆc §¹i §Çu LÜnh", 1)
if nNpcIndex > 0 then
SetNpcDeathScript(nNpcIndex,"\\script\\missions\\fengling_ferry\\boss.lua");
SetNpcTimer(nNpcIndex,900*18)
end
end

function thuytacdaulinh2()
nNpcIndex = AddNpc(1692, 85,SubWorldID2Idx(338), 1634*32, 3221*32, 1, "Thñy TÆc §¹i §Çu LÜnh", 1)
if nNpcIndex > 0 then
SetNpcDeathScript(nNpcIndex,"\\script\\missions\\fengling_ferry\\boss2.lua");
SetNpcTimer(nNpcIndex,900*18)
end
end
...
tbDropTemplet:GiveAwardByList(nNpcIndex, -1, tbItem1,TitleStr, 1)
	local szNews = format("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().." ®· Tiªu diÖt  thµnh c«ng §¹i Thñy TÆc §Çu LÜnh => BÕn 1 Phong L¨ng §é");
