...
function tbAwardTemplet:GiveWithoutLog(tbItem, nAwardCount)
    if not tbItem then
        return 0
    end
    ...
    if type(tbItem[1]) == "table" then
        if tbItem[1].nRate then
            for i = 1, nAwardCount do
                self:GivByRandom(tbItem, 1)
            end
            return 1
        else
            for i = 1, getn(tbItem) do
                self:GiveWithoutLog(tbItem[i], nAwardCount)
            end
            return 1
        end
    else
        for k, v in self.TYPE do
            if tbItem[k] then
                v:GiveWithoutLog(tbItem, nAwardCount)
                return 1
            end
        end
    end
end
function tbAwardTemplet:GiveAwardByListWithNoneLog(tbItem, nAwardCount)
    return self:GiveWithoutLog(tbItem, nAwardCount)
end
