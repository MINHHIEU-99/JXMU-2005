...
------------xoa thuoc lag
local nTime = 60*18*5
local tbSkill = { 1120,261,258,260,1121,256,259,257} 
for i=1,getn(tbSkill) do 
RemoveSkillState(tbSkill[i], 1, 1, nTime,1) 
end 	
...
function bt_enterbattle()
	...
	if (mem_song == nil or mem_jin == nil) then
		...
		--------check ip --- tru thien tu
	local nTime = tonumber(GetLocalDate("%H%M"))
		local nWeek = tonumber(GetLocalDate("%w"))
		if nWeek==1 and nTime>=2050 and nTime<=2200 then
