...
dofile("script/gm_tool/lenhbaihotro.lua")
...
local tbOpt = 
{
	...
	---{"Mua 1000 M¸u Tèng Kim (2 xu)",Mua500MauTongKim},
	--{"T¨ng TiÒm N¨ng Nhanh", TangDiemTiemNangNhanh},
	{"Xem Th«ng Tin Nh©n VËt", xemsachvo},
	{"NhËn 1 BÝ KÝp 9x (CÊp 80 nhËn)",nhanbikip9x},
	--{"NhËn 3 BÝ KÝp 9x (CÊp 90 nhËn)",nhanbikip9x2},
	--{"Mua 10 Cµn Kh«n T¹o Hãa §an(500 §iÓm Tèng Kim)",MuaCanKhonTaoHoaDan},
	{"NhËn Vßng S¸ng T©n Thñ §Õn CÊp 83", nhanvongsang1},
	...
	{"Hñy VËt PhÈm Kho¸ VÜnh ViÔn",huyVPkhoa},
	...
	-- if GetAccount() == "treamerkyduyen" then
	--tinsert(tbOpt, 1, {"GM Thay §æi Tr¹ng Th¸i", trangthai22}) 
	...
	if GetLevel()>=80 then
		if GetTask(3031)==0 then
			SetTask(3031,1)
				local award1111  ={
{
--	{szName="§¹i Thµnh BÝ Kip 9x",tbProp={6,1,2424,1,1},nCount=1,nBindState=-2},
...
function nhanbikip9x2()
if CalcFreeItemCellCount() < 5 then
		return Talk(1,"","H·y S¾p XÕp L¹i Hµnh Trang")
end
	if GetLevel()>=90 then
		if GetTask(3031)==1 then
			SetTask(3031,2)
				local award1111  ={
{
--	{szName="§¹i Thµnh BÝ Kip 9x",tbProp={6,1,2424,1,1},nCount=1,nBindState=-2},
		{szName="BÝ Kip 9x",tbProp={6,1,2426,1,1},nCount=3,nBindState=-2},
	},
	
}
tbAwardTemplet:GiveAwardByList(award1111, "PhÇn Th­ëng");
		else
		Say("Mçi nh©n vËt chØ nhËn ®­îc 1 lÇn duy nhÊt hoÆc ®¹i hiÖp ch­a nhËn th­ëng bÝ kÝp cÊp 80.")
		return
		end
	else
		Say("CÊp ®é 90 míi nhËn ®­îc.")
		return
	end
...
local nValue	= GetTask(1882);
 ...
 Say("Vâ l©m mËt tÞch: <color=cyan>"..GetTask(80).."<color>\nTÈy Tuû Kinh:  <color=cyan>"..GetTask(81).."<color>\nTrÊn Ph¸i Linh §¬n:  <color=cyan>"..nTimes.."<color>\nTrÊn Ph¸i Linh D­îc:  <color=cyan>"..nTimes2.."<color>\nCèng NguyÖt Phï Dung:  <color=cyan>"..GetTask(701).."<color>\nPhông NguyÖt Qu¶ Dung:  <color=cyan>"..GetTask(700).."<color>")
end
...
function nhanvongsang1()
	...
	if GetSkillState(1512)>=1 then
		...
		PlayerFunLib:AddSkillState(1512,20,3,18*60*30,1)
		PlayerFunLib:AddSkillState(1679,20,3,18*60*30,1)
end
function trangthai1()
	local tbOpt =
	{
...
local tbAwardcc={
{szName="Cµn Kh«n T¹o Hãa §an", tbProp={6,1,215,1,0,0},nCount = 1000},
}
	local nCount_muaban = CalcEquiproomItemCount(4,417,1,1)
	if  nCount_muaban >= 2  then
		ConsumeEquiproomItem(2, 4,417, 1,-1)
		tbAwardTemplet:GiveAwardByList(tbAwardcc, "PhÇn Th­ëng");
		Talk(1, "", "Trao ®æi thµnh c«ng nhËn ®­îc <color=yellow><1000 Cµn Kh«n T¹o Hãa §an><color>");
	else
		Say("<#> Trao ®æi vËt phÈm cÇn cã <color=yellow>2 xu <color>, cã ®ñ tiÒn råi ®Õn nhÐ!", 0)
	end
