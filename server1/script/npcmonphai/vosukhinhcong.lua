...
--	Task0025 = GetTaskTemp(25)			-- ´òÄ¾×®/É³´ü/Ä¾ÈË¼ÆÊý(ÁÙÊ±±äÁ¿)
	...
	if ( Uworld1067 < 10 ) and ( Uworld1000 > 200 ) then		
		--tbDailog.szTitleMsg = DescLink_WuShi.."<#>: Trªn nói Nam S¬n cã tuyÕt, giã l¹nh bèn mïa, t«i giµ råi, kh«ng cã tuæi trÎ n÷a ®Ó ch¬i nh÷ng trß nµy, con ng­êi còng vËy, nhiÒu lóc lo l¾ng, lµm viÖc mµ kh«ng cã sù nghiªm tóc. µ, kh«ng nãi vÒ chuyÖn nµy n÷a. B¹n t×m t«i cã viÖc g× kh«ng?"
		--tbDailog:AddOptEntry("T«i ®ang lµm nhiÖm vô häc tËp, ®Õn t×m b¹n gióp ®ì", education_wushi)
		--tbDailog:AddOptEntry("T«i cã mét nhiÖm vô b¸n thÞt thó", education_collectmeat)
		--tbDailog:AddOptEntry("T«i muèn häc kü n¨ng bay", education_fly)
		tbDailog:AddOptEntry("T«i muèn chuyÓn ®æi kü n¨ng Ngò hµnh", education_changewuxing)
		--tbDailog:AddOptEntry("Kh«ng cã g×, chØ ®i ngang qua th«i", no)
...
function education_givemeat()
	...
	if ( Uworld1066 > 0 ) then
		Describe(DescLink_WuShi.."<#>:Ng­¬i ®· nhËn nhiÖm vô t×m thÞt t­¬i råi! Kh«ng cÇn lµm n÷a!"..Uworld1066.."miÕng thÞt t­¬i ®i! H·y Ên phÝm <color=red>F12<color> ®Ó xem h­íng dÉn chi tiÕt vÒ nhiÖm vô!","KÕt thóc ®èi tho¹i/no")
	else
		Uworld1066 = random(5,20)
		nt_setTask(1066,Uworld1066)
		Describe(DescLink_WuShi.."<#>:HiÖn giê ta cÇn"..Uworld1066.."miÕng thÞt t­¬i, h·y ®i t×m gióp ta!.",1,"KÕt thóc ®èi tho¹i/no")
	end
end
