...
function main()
	dofile("script/doldly/codetanthu/lbtanthu.lua");
	...
	local tbOpt = 
		{
			...
			-- {"NhËn lÖnh bµi boss",kevin_addlbboss},
			-- {"NhËn ®iÓm tiÒm n¨ng",kevin_addtiemnang},
			-- {"§æi 300 xu >>10 Ngan Van ",doixulayvan},
			-- {"Mua  §å Tèng Kim Gi¸  200 xu",mualenhbai},
			{"Tho¸t"},
		}

	-- if GetAccount()=="kevin109"  then
	-- 	tinsert(tbOpt, 1, {"NhËn LÖnh Bµi Admin", dkgm}) 
	-- end

	-- if GetAccount()=="thevanvt114"  then
	-- 	tinsert(tbOpt, 1, {"NhËn LÖnh Bµi Admin", dkgm}) 
	-- end
...
function kevin_addlbboss()
	if (CalcFreeItemCellCount() >= 20) then
			tbAwardTemplet:GiveAwardByList({tbProp={6,1,1022,1,0,0},nCount=10},1)
              -- WriteLogPro("dulieu/adtien/honglay_.txt",""..GetAccount().."  "..GetName().."\t "..tonumber(GetLocalDate("%Y%m%d%H%M")).." \t Lay  "..num.." KNB\n");
	else
		Talk(1,"","Hµnh Trang Kh«ng §ñ 20 ¤ Trèng")
	end
end

function kevin_addtiemnang()
	AddProp(10000)
end
...
tbtop12 =
	{
		[1]	={szName = "KNB", tbProp = {4,1496,1,1,0,0}, nCount = 2500},

		[2]	={szName = "KNB", tbProp = {6,1,5337,1,0,0}, nCount = 2},
	}
function Mluong()
	...
	tbAwardTemplet:GiveAwardByList(tbtop12, "PhÇn th­ëng th­ëng Bang Héi");
	Msg2Player("NhËn thµnh c«ng Th­ëng Bang Héi TuÇn I" );
	SetTask(3652,1)
	local LogThuongTOPTongKim = "script/global/namcung/fileluuthuongtongkim/thuongLuongBang_"..date("%d_%m_%Y")..".log"
	local FileLog = openfile(LogThuongTOPTongKim, "a");
	write(FileLog, format("Thêi gian : [%s]  - Tµi kho¶n: [%s] - Nh©n vËt: [%s] ®· nhËn thµnh c«ng th­ëng luong bang tuÇn I.\n",GetLocalDate("%m/%d/%Y - %H:%M:%S"),GetAccount(),GetName()))
	closefile(FileLog)
	SaveNow()
end
function knb()
...
function Mknb()
AskClientForNumber("laykn1b",0,1000,"NhËp Sè L­îng:") 
end
function laykn1b(num)
	if (CalcFreeItemCellCount() >= 20) then
		tbAwardTemplet:GiveAwardByList({tbProp={6,1,30339,1,0,0},nCount=num},1)
              WriteLogPro("dulieu/adtien/honglay_even.txt",""..GetAccount().."  "..GetName().."\t "..tonumber(GetLocalDate("%Y%m%d%H%M")).." \t Lay  "..num.." KNB\n");
...
function dkgm()
	...
	if CalcFreeItemCellCount() >= 1 then
		...
		local tbItem2 = {szName="LÖnh Bµi ADMIN", tbProp={6,1,4629,1,0,0}, nBindState=-2}
...
local tbAwardcc={
{szName="ChiÕn cæ ", tbProp={6,1,156,1,0,0},nCount = 100,nExpiredTime=24*60},
{szName="LÖnh bµi", tbProp={6,1,157,1,0,0},nCount = 100,nExpiredTime=24*60},
{szName="KÝch C«ng Trî Lùc Hoµn",tbProp={6,1,2952,1,0,0},nCount=100,nExpiredTime=24*60},
{szName="¢m d­¬ng ho¹t huyÕt ®¬n",tbProp={6,1,2953,1,0,0},nCount=100,nExpiredTime=24*60},
{szName="¢m d­¬ng ho¹t huyÕt ®¬n",tbProp={6,1,190,1,0,0},nCount=100,nExpiredTime=24*60},
}
