...
return end
	if (nDate >= 20180512 and nDate <= 20220730 and nTime >= 0000 and nTime <= 2359) then
		if GetTask(HOAPHUONGTIM) < 1500 then
			if CalcFreeItemCellCount() >= 10 then
				SetTask(HOAPHUONGTIM,GetTask(HOAPHUONGTIM)+1) tbAwardTemplet:GiveAwardByList(tbAward,"PhÇn Th­ëng B¸nh Sinh NhËt")
			else
				Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y sö dông",0) return 1
			end
		else
			Say(""..myplayersex().." §· Sö Dông Tèi §a 1500 B¸nh Sinh NhËt­\nTrong Suèt Thêi Gian Ho¹t §éng") return 1
		end
	else
