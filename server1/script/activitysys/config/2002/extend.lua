...
Include("\\script\\activitysys\\config\\2002\\data.lua")
...
pActivity.tbStartDay = {2012,3,29,10,0} -- ?Ô2012/3/1 10£º00 ÎªÖÜËÄµÄÆdÊ¼Ê±¼ä
pActivity.nTempTime = 7 * 24 * 60 * 60		-- A¿ÖÜÅÅAû?»´Î
...
function pActivity:AddAmbienceNpc()
	Include("\\script\\activitysys\\config\\2002\\ambience_npc.lua")
...
-- ¸üÐÂ×î½ü?»´ÎµÇÂ¼Ê±¼ä£¬Èôµ±`´ÓÐµÇÂ¼£¬Ô?²»¸Ä±ä£¬
-- ·ñÔ?£¬½«×î½ü?»´ÎµÇÂ¼Ê±¼ä¸ÄÎªµ±`´µÄÔçÉÏ8µa
...
-- ¸ù¾Urelay·µ»ØµÄÏûÏ¢ÅÐ¶Ïµ±Ç°Íæ¼?ÊÇ²»ÊÇÖ÷Ë§
...
function pActivity:GetTitle()
	...
	SetTask(TSK_ACTIVE_TITTLE, n_title)	-- ÉèÖAÏÂ´ÎÉÏÏßÖ®ºó»¹ÊÇOâ¸ö³ÆºÅ

	%tbLog:PlayerAwardLog(%EVENT_LOG_TITLE, "NhanDanhHieuChuSoai")
	self:AddTaskDaily(%TSK_DAILY_GET_TITLE, 1)
	Msg2Player("Chóc mong ®¹i hiÖp nhËn ®­îc danh hiÖu §Ö NhÊt Chñ So¸i")
end
...
function pActivity:DoActiveFunction(nParam)
	...
	if GetCash() < nJxb then
		Msg2Player(format("KUch ho¹t tUnh n¨ng nµy cÇn ph¶i ca %d ng©n l­îng, h·y chuÈn b~ kü råi quay l¹i",nJxb))
		return
	end
	Pay(nJxb)
...
function pActivity:ActiveFunction_2(nParam, ResultHandle)
	...
	if nResult ~= 1 then
		CallPlayerFunction(nPlayerIndex,Say,"Ng­¬i kh«ng ph¶i chñ so¸i cña tuÇn nµy, kh«ng thÓ kUch ho¹t tUnh n¨ng nµy")
		return
	end
...
--	print("nStartTime = " .. nTime)
	-- ¸üÐÂÏÂ´Î¸üÐÂÊ±¼ä
	-- ¼ÆËaµ±Ç°Ê±¼äºÍ2012/3/1 10:00 ²îÁË¼¸ÖÜ£¬¸ù¾U²îÖµ£¬¼ÆËaÏÂ´Î¸üÐÂÅÅAûµÄÊ±¼ä
...
-- ÅÐ¶ÏÊÇ·ñÐè?ªÇå¿O»u·Ö£¬Èç¹ûÐè?ªÔ?Çå¿O
...
--	print("nCurrTime = " .. nCurrTime)
--	print("nNextSortTime = " .. nNextSortTime)
	-- µ±Ç°µÄÊ±¼äÐ¡ÓÚÏÂ´Î¸üÐÂÊ±¼ä£¬Ô?²»´¦Àí
	if nCurrTime <= nNextSortTime then
		return 0
	end
	-- Çå¿Oµ±Ç°»u·Ö,¸üÐÂÏÂ´ÎÇå¿OÊu¾UµÄÊ±¼ä
...
function pActivity:QueryMark_2(nParam, ResultHandle)
	...
	CallPlayerFunction(nPlayerIndex, Say, format("TUch l?y danh hiÖu hiÖn t¹i cña c¸c h¹ lµ %d, ®iÓm tUch l?y cao nhÊt hiÖn t¹i lµ %d", nPlayerMark, nMaxMark))
end
	...
	for k=1,nCount do
		...
		for i=1,getn(tbMark)do
			...
			if nRate <= nTotalRate then
				...
				Msg2Player(format("Danh hiÖu vµ tUch l?y cña ng­¬i ®· t¨ng lªn %d",tbMark[i].nMark))
				break
			end
		end
...
function pActivity:PlayerSignUp(nType)
	...
	-- ÖÜ?»£¬¶?£¬ËÄ£¬ÁùµÄ21:00µaËÎ½d²»ÄÜ¼¤»î´Ë½±Àø
	if bResult == 0 then
		return
	end
