...
tbMaterial_Shijin4 = {szName="§¹i hû lÔ bao",tbProp={6,1,1760,1,0,0},nExpiredTime=nTime,nCount=1,}
	
tbProduct = {
	[1] = {szName="B¸nh Kem C¸t T­êng",tbProp={6,1,1762,1,0,0},nExpiredTime=nTime,},
	}
...
for i=1,n_key do
DynamicExecuteByPlayer(PlayerIndex, "\\script\\activitysys\\config\\2006\\update_xunzhang.lua",  "tbProgressBar:OpenByConfig", 14, self.WantCompose, {self, GetName()})
end
end
...
function tbUpdateXunZhang:Compose()
	...
	if self:ConsumeMaterial(tbMaterial, nComposeCount, self.szLogTitle) ~= 1 then
		Msg2Player("Ch? t¹o thÊt b¹i, mÊt ®i mét sè nguyªn liÖu.")
		return 0;
	end
	...
	if bSuccess == 0 then
		...
		Msg2Player("Chóc mong, n©ng cÊp thµnh c«ng")
	end
...
function pActivity:InitUpdateCompose()
	...
	for nType=1,getn(%tbMaterial_Medal) do
		...
		tbFormulaList.nRate = %TyLeBanhKemCatTuong[nType]
