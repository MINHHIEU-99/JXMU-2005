...
----------------------------------------
function main()
	dofile("script/global/g7vn/event/quabong/eventmain.lua")
	...
	if (nDate >= startdate and nDate <= enddate and nTime >= 0000 and nTime <= 2359) then
		...
		tinsert(tbSay,"NhËn Th­ëng Mèc Cup V« §Þch./NhanMoctask_cupvodich")
	--	tinsert(tbSay,"NhËn Th­ëng Mèc Hép quµ Chóc Mõng./NhanMocBanhThapCam")
...
--		tinsert(tbSay,"Show Top Event./ShowTopThiep")
		...
		Say("Chµo Mõng Nh©n SÜ Tham Gia event trung thu cïng VoLamVietPK. B¹n ®· sö dông: <color=red>"..GetTask(task_cupvodich).." <color>Cup V« §Þch", getn(tbSay), tbSay)
		--end
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function NhanMoctask_cupvodich()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_cupvodich).." <color>Cup V« §Þch", getn(tbSay), tbSay)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_hopquachucmung).." <color>Hép quµ Chóc Mõng", getn(tbSay), tbSay)
...
function Moc600ThapCam()
	...
	if (GetTask(task_hopquachucmung) >= 600) and (GetTask(task_moc_hopquachucmung) == 0) then
		local tbAward = {
		...
		tbAwardTemplet:GiveAwardByList(tbAward,"PhÇn Th­ëng Mèc 500") SetTask(task_moc_hopquachucmung,1)
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 600 Hép quµ Chóc Mõng")
...
if CalcFreeItemCellCount() < 20 then
	...
	if (GetTask(task_hopquachucmung) >= 1200)  and (GetTask(task_moc_hopquachucmung) == 1) then
		local tbAward = {
			[1] = {
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event Hép quµ Chóc Mõng 1200")
		tbAwardTemplet:GiveAwardByList(tbAward,"PhÇn Th­ëng Mèc 400") SetTask(task_moc_hopquachucmung,2)
	else
		Talk(1,"","B¹n Ch­a §¹t Tíi Mèc Nµy")
	end
end
...
function Moc500()
	...
	if (GetTask(task_cupvodich) >= 600) and (GetTask(task_nhanmoccupvodich) == 0) then
		local tbAward = {
			{szName="LÔ bao HuyÕt ChiÕn (§¹i)",tbProp={6,1,4898,1,0,0},nCount=1},	
			{szName="Tinh S­¬ng Th¹ch (®¹i)",tbProp={6,1,4881,1,0,0},nCount=5},	
			{szName="B¸ch LuyÖn Thµnh C­¬ng",tbProp={6,1,4906,1,0,0},nCount=1},	
		...
		tbAwardTemplet:GiveAwardByList(tbAward,"PhÇn Th­ëng Mèc 600") SetTask(task_nhanmoccupvodich,1)
...
local Xu = CalcEquiproomItemCount(4,417,1,1)
	if (GetTask(task_cupvodich) >= 1000) and (GetTask(MOCRESETEVENT)) ==0 then
		if (Xu >= 0) then
			SetTask(task_cupvodich,0)-- reset banh dac biet ve 0
			SetTask(task_nhanmoccupvodich,0)  --reset moc banh db ve 0
...
if CalcFreeItemCellCount() < 20 then
	...
	if (GetTask(task_cupvodich) >= 1200)  and (GetTask(task_nhanmoccupvodich) == 1)then
		local tbAward = {
			[1] = {
		...
		tbAwardTemplet:GiveAwardByList(tbAward,"PhÇn Th­ëng Mèc 400") SetTask(task_nhanmoccupvodich,2)
				local a = random(1,10)
				if a==11 then
