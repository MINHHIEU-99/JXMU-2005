...
enddate=20191220
function main()
	dofile("script/global/g7vn/event/quabong/chiecgiayvang.lua")
...
--	local task_chiecgiayvang=4997
	if (GetTask(task_chiecgiayvang) >= 1200) then
...
tbAwardTemplet:GiveAwardByList({{nExp_tl = 100e6}}, "test", 1);
	...
	SetTask(task_chiecgiayvang,GetTask(task_chiecgiayvang)+1)
	Msg2Player("§· sö dông lÇn thø: <color=green>"..GetTask(task_chiecgiayvang) .."<color>")
	return 0
end
