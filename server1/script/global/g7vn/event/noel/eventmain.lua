...
----------------------------------------
function main()
	dofile("script/global/g7vn/event/noel/eventmain.lua")
	...
	if (nDate >= startdate and nDate <= enddate and nTime >= 0000 and nTime <= 2359) then
		...
		tinsert(tbSay,"NhËn Th­ëng Hép quµ gi¸ng sinh./NhanMoc")
		tinsert(tbSay,"NhËn Th­ëng Mèc Hép quµ an lµnh./NhanMocBanhThapCam")
	 tinsert(tbSay,"Mua R­¬ng Hép quµ gi¸ng sinh./MuaEvent")
	...
	tinsert(tbSay,"Show Top Event./ShowTopThiep")
		...
		Say("Chµo Mõng Nh©n SÜ Tham Gia event trung thu cïng VoLamVietPK. B¹n ®· sö dông: <color=red>"..GetTask(task_hopquagiangsinh).." <color>Hép quµ gi¸ng sinh", getn(tbSay), tbSay)
		--end
...
local tbSay = {}
		...
		tinsert(tbSay,"Mua 1 R­¬ng Hép quµ gi¸ng sinh <25xu - 50 b¸nh> ./muahop1")
	--	tinsert(tbSay,"Mua 10 R­¬ng Hép quµ gi¸ng sinh <250xu - 500 b¸nh> ./muahop10")
...
function NhanMoc()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_hopquagiangsinh).." <color>Hép quµ gi¸ng sinh", getn(tbSay), tbSay)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_hopquaanlanh).." <color>Hép quµ an lµnh", getn(tbSay), tbSay)
...
function Moc600ThapCam()
	...
	if (GetTask(task_hopquaanlanh) >= 600) and (GetTask(task_moc_hopquaanlanh) == 0) then
		local tbAward = {
		...
		tbAwardTemplet:GiveAwardByList(tbAward,"PhÇn Th­ëng Mèc 500") SetTask(task_moc_hopquaanlanh,1)
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 600 Hép quµ an lµnh")
...
if CalcFreeItemCellCount() < 20 then
	...
	if (GetTask(task_hopquaanlanh) >= 1200)  and (GetTask(task_moc_hopquaanlanh) == 1) then
		local tbAward = {
			[1] = {
		...
		WriteLogPro("dulieu/event_moc2.txt",""..GetAccount().."  "..GetName().."\t "..tonumber(GetLocalDate("%Y%m%d%H%M")).."   "..GetIP().."\t Da An max Moc 1000 Event tien van\n");
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event Hép quµ an lµnh 1200")
		tbAwardTemplet:GiveAwardByList(tbAward,"PhÇn Th­ëng Mèc 400") SetTask(task_moc_hopquaanlanh,2)
	else
		Talk(1,"","B¹n Ch­a §¹t Tíi Mèc Nµy")
	end
end
...
function Moc500()
	...
	if (GetTask(task_hopquagiangsinh) >= 600) and (GetTask(task_nhanmoc_hopquagiangsinh) == 0) then
		local tbAward = {
			{szName="LÔ bao HuyÕt ChiÕn (§¹i)",tbProp={6,1,4898,1,0,0},nCount=1},	
		...
		tbAwardTemplet:GiveAwardByList({{nExp_tl = 80e9}}, "test", 1);
		tbAwardTemplet:GiveAwardByList(tbAward,"PhÇn Th­ëng Mèc 600") SetTask(task_nhanmoc_hopquagiangsinh,1)
...
local Xu = CalcEquiproomItemCount(4,417,1,1)
	if (GetTask(task_hopquagiangsinh) >= 1000) and (GetTask(MOCRESETEVENT)) ==0 then
		if (Xu >= 0) then
			SetTask(task_hopquagiangsinh,0)-- reset banh dac biet ve 0
			SetTask(task_nhanmoc_hopquagiangsinh,0)  --reset moc banh db ve 0
...
if CalcFreeItemCellCount() < 20 then
	...
	if (GetTask(task_hopquagiangsinh) >= 1200)  and (GetTask(task_nhanmoc_hopquagiangsinh) == 1)then
		local tbAward = {
			[1] = {
		...
		tbAwardTemplet:GiveAwardByList({{nExp_tl = 120e9}}, "test", 1);
		...
		tbAwardTemplet:GiveAwardByList(tbAward,"PhÇn Th­ëng Mèc 400") SetTask(task_nhanmoc_hopquagiangsinh,2)
				local a = random(1,10)
				if a==11 then
