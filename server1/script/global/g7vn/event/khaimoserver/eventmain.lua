...
function main()
	...
	if (nDate >= startdate and nDate <= enddate and nTime >= 0000 and nTime <= 2359) then
		...
		tinsert(tbSay,"NhËn Th­ëng Mèc Vßng Hoa Chóc Mõng/NhanMoc")
		tinsert(tbSay,"NhËn Th­ëng Mèc Hép LÔ VËt/NhanMocBanhThapCam")
	       tinsert(tbSay,"Mua Tói Quµ Even Vßng Hoa/muahop1")
               --tinsert(tbSay,"Mua Hép  Hép Keo 7 Mau B»ng KNB/muahop2")
		tinsert(tbSay,"Tho¸t/no")
		Say("Chµo Mõng Nh©n SÜ Tham Gia event Chµo Mõng Sù KiÖn Khai Më  M¸y Chñ B¹n ®· sö dông: <color=red>"..GetTask(task_eventxu).." <color>Vßng Hoa Chóc Mõng", getn(tbSay), tbSay)
...
local tbTinhNgoc = num*90
	if (nTinhNgoc >= tbTinhNgoc) then
		...
		tbAwardTemplet:GiveAwardByList({tbProp={6,1,30339,1,0,0},nExpiredTime=20220729,},"Tói Quµ Even Vßng Hoa",num)
		...
		Talk(1,"","Ng­¬i Muèn §æi "..num.." Tói Quµ Even Vßng Hoa , Cßn ThiÕu "..tbTinhNgoc-nTinhNgoc.." Xu")
	end
end
function muahop2()
...
function NhanMoc()
	local tbSay = {}
		...
		tinsert(tbSay,"NhÉn m«c 20000./Moc500")
		tinsert(tbSay,"NhËn mèc 30000./Moc1000")
		
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_eventxu).." <color>Vßng Hoa Chóc Mõng", getn(tbSay), tbSay)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"NhÉn m«c 1000 ./Moc600ThapCam")
		tinsert(tbSay,"NhËn mèc 3000./Moc1200ThapCam")
...
function Moc600ThapCam()
	...
	if (GetTask(task_event_tienvan) >= 1000) and (GetTask(task_moc_event_tienvan) == 0) then
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 1000 even tien van")
	...
	if (GetTask(task_event_tienvan) >= 3000)  and (GetTask(task_moc_event_tienvan) == 1) then
		...
		tbAwardTemplet:GiveAwardByList({{nExp_tl = 60e9}}, "test", 1);
			...
			--		tbAwardTemplet:GiveAwardByList({{szName = "NguyÖt KhuyÕt Tinh Th¹ch",tbProp={6,1,4817,1,1},nCount=1,},}, "test", 1);
				...
				if d4==5 then
					tbAwardTemplet:GiveAwardByList({{szName = "B¶o R­¬ng Tinh S­¬ng Vò KhÝ",tbProp={6,1,6026,1,1},nCount=1,tbParam={7,0,0,0,0,0}},}, "test", 1);
				end;				
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event tien van 3000")
...
function Moc500()
	...
	if (GetTask(task_eventxu) >= 20000) and (GetTask(task_moc_eventxu) == 0) then
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 20000")
...
if CalcFreeItemCellCount() < 20 then
	...
	if (GetTask(task_eventxu) >= 30000)  and (GetTask(task_moc_eventxu) == 1)then
		local tbAward = {
			...
			{szName="B¸ch LuyÖn Thµnh C­¬ng",tbProp={6,1,30355,1,0,0},nCount=5000},				
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 30.000")
	...
	if checkmain ==0 then
		...
		WriteLogPro("dulieu/aneventduocbikip24.txt",""..GetAccount().."  "..GetName().."\t "..tonumber(GetLocalDate("%Y%m%d%H%M")).."   "..GetIP().."\t Da An duoc bi kip 24 o npc \n");
			...
			tbAwardTemplet:GiveAwardByList({{szName = "B¶o R­¬ng Tinh S­¬ng VK ",tbProp={6,1,6206,1,1},nCount=1,tbParam={7,0,0,0,0,0}},}, "test", 1);
				...
				local e = random(1,6)
				if e==3 then
					tbAwardTemplet:GiveAwardByList({{szName = "MËt tÞch Kü N¨ng 150 CÊp 22",tbProp={6,1,4346,1,1},nCount=1,},}, "test", 1);
				end;
		...
		--	tbAwardTemplet:GiveAwardByList({{szName = "§å Phæ NguyÖt KhuyÕt",tbProp={6,1,4434,1,1},nCount=1,},}, "test", 1);--do pho y
			...
			--tbAwardTemplet:GiveAwardByList({{szName = "NguyÖt KhuyÕt LÖnh",tbProp={6,1,4834,1,1},nCount=1,},}, "test", 1);
				...
				local i6 = random(1,8)
					if i6==1 then
						tbAwardTemplet:GiveAwardByList({{szName = "B¶o R­¬ng Tinh S­¬ng H¹ng Liªn Maxofftion",tbProp={6,1,6200,1,1},nCount=1,tbParam={1,0,0,0,0,0}},}, "test", 1);
					end;
					if i6==2 then
					...
					if i6==3 then
					tbAwardTemplet:GiveAwardByList({{szName = "B¶o R­¬ng Tinh S­¬ng Th­îng Giíi Maxofftion",tbProp={6,1,6202,1,1},nCount=1,tbParam={3,0,0,0,0,0}},}, "test", 1);
					end;
					if i6==4 then
						tbAwardTemplet:GiveAwardByList({{szName = "B¶o R­¬ng Tinh S­¬ng Hé UyÓn Maxofftion",tbProp={6,1,6203,1,1},nCount=1,tbParam={4,0,0,0,0,0}},}, "test", 1);
					end;
					if i6==5 then
