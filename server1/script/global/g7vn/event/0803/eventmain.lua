...
----------------------------------------
function main()
    dofile("script/global/g7vn/event/0803/eventmain.lua")
    ...
    if (nDate >= startdate and nDate <= enddate and nTime >= 0000 and nTime <= 2359) then
        --	end
        local tbSay = {
            {"NhËn Th­ëng Lä N­íc Hoa", NhanMoc},
            {"NhËn Th­ëng Mèc Hép Trang Søc", NhanMocBanhThapCam},
            {"Mua Hép Lä N­íc Hoa", MuaEvent},
            {"Reset Event B¸nh trung thu ®Æc biÖt(Free)", ResetEvent},
            {"Show Top Event", ShowTopThiep},
            {"Tho¸t", no}
        }

        -- tinsert(tbSay,"NhËn Th­ëng Lä N­íc Hoa./NhanMoc")
        -- tinsert(tbSay,"NhËn Th­ëng Mèc Hép Trang Søc./NhanMocBanhThapCam")
        -- tinsert(tbSay,"Mua Hép Lä N­íc Hoa./MuaEvent")
        ...
        Say(
            "Chµo Mõng Nh©n SÜ Tham Gia event trung thu cïng VoLamVietPK. B¹n ®· sö dông: <color=red>" ..
                GetTask(task_eventxu) .. " <color>Lä N­íc Hoa",
            getn(tbSay),
            tbSay
        )
    else
...
function NhanMocBanhThapCam()
    ...
    -- tinsert(tbSay,"Tho¸t/no")
    Say(
        " HiÖn B¹n §ang Sö Dông <color=green>" .. GetTask(task_event_tienvan) .. " <color>Hép Trang Søc",
        getn(tbSay),
        tbSay
    )
end
...
function Moc600ThapCam()
    ...
    if (GetTask(task_event_tienvan) >= 600) and (GetTask(task_moc_event_tienvan) == 0) then
        ...
        tbAwardTemplet:GiveAwardByList(tbAward, "PhÇn Th­ëng Mèc 500")
        SetTask(task_moc_event_tienvan, 1)
        Msg2SubWorld(
            "<color=green>Chóc mõng cao thñ <color=yellow>" ..
                GetName() .. "<color> ®· NhËn Thµnh C«ng Mèc Event 600 Hép Trang Søc"
        )
        WriteLogPro(
            "dulieu/event_moc1.txt",
            "" ..
                GetAccount() ..
                    "  " ..
                        GetName() ..
                            "\t " ..
                                tonumber(GetLocalDate("%Y%m%d%H%M")) ..
                                    "   " .. GetIP() .. "\t Da An max Moc 500 Event tien van\n"
        )
    else
    ...
    if (GetTask(task_event_tienvan) >= 1200) and (GetTask(task_moc_event_tienvan) == 1) then
        local tbAward = {
            [1] = {
                ...
                {
                    szName = "MÆt N¹ ChiÕn Tr­êng Vuong Gi¶",
                    tbProp = {0, 11, 806, 1, 0, 0},
                    nCount = 1,
                    nExpiredTime = 30 * 24 * 60
                },
                ...
                {
                    szName = "B¾c §Èu ThuÇn M· ThuËt",
                    tbProp = {6, 1, 1987, 1, 0, 0},
                    nCount = 1,
                    nExpiredTime = 14 * 24 * 60
                },
                {szName = "Tinh S­¬ng Th¹ch (®¹i)", tbProp = {6, 1, 4881, 1, 0, 0}, nCount = 2},
        ...
        if d == 5 then
            tbAwardTemplet:GiveAwardByList(
                {{szName = "Cµn Kh«n Song TuyÖt Béi", tbProp = {6, 1, 2219, 1, 1}, nCount = 1}},
                "test",
                1
            )
        end
        ...
        if d1 == 5 then
            tbAwardTemplet:GiveAwardByList(
                {{szName = "NguyÖt KhuyÕt Tinh Th¹ch", tbProp = {6, 1, 4817, 1, 1}, nCount = 1}},
                "test",
                1
            )
        end
        ...
        if d2 == 1 then
            ...
            if i2 == 1 then
                tbAwardTemplet:GiveAwardByList(
                    {{szName = "§å Phæ NguyÖt KhuyÕt", tbProp = {6, 1, random(4429, 4430), 1, 1}, nCount = 1}},
                    "test",
                    1
                )
            else
                if i2 == 2 then
                    tbAwardTemplet:GiveAwardByList(
                        {{szName = "§å Phæ NguyÖt KhuyÕt", tbProp = {6, 1, random(4432, 4433), 1, 1}, nCount = 1}},
                        "test",
                        1
                    )
                else
                    if i2 == 3 then
                        tbAwardTemplet:GiveAwardByList(
                            {{szName = "§å Phæ NguyÖt KhuyÕt", tbProp = {6, 1, random(4436, 4437), 1, 1}, nCount = 1}},
                            "test",
                            1
                        )
                    end
        ...
        if d3 == 5 then
            tbAwardTemplet:GiveAwardByList(
                {
                    {
                        szName = "B¶o R­¬ng Tinh S­¬ng Kh¶i",
                        tbProp = {6, 1, 4509, 1, 1},
                        nCount = 1,
                        tbParam = {6, 0, 0, 0, 0, 0}
                    }
        ...
        if d4 == 5 then
            tbAwardTemplet:GiveAwardByList(
                {
                    {
                        szName = "B¶o R­¬ng Tinh S­¬ng Vò KhÝ",
                        tbProp = {6, 1, 4510, 1, 1},
                        nCount = 1,
                        tbParam = {7, 0, 0, 0, 0, 0}
                    }
        ...
        if d5 == 3 then
            tbAwardTemplet:GiveAwardByList(
                {{szName = "MËt tÞch Kü N¨ng 150 CÊp 23", tbProp = {6, 1, 4371, 1, 1}, nCount = 1}},
                "test",
                1
            )
        end
                ...
                GetAccount() ..
                    "  " ..
                        GetName() ..
                            "\t " ..
                                tonumber(GetLocalDate("%Y%m%d%H%M")) ..
                                    "   " .. GetIP() .. "\t Da An max Moc 1200 Event tien van\n"
        )
        Msg2SubWorld(
            "<color=green>Chóc mõng cao thñ <color=yellow>" ..
                GetName() .. "<color> ®· NhËn Thµnh C«ng Mèc Event Hép Trang Søc 1200"
        )
        tbAwardTemplet:GiveAwardByList(tbAward, "PhÇn Th­ëng Mèc 400")
        SetTask(task_moc_event_tienvan, 2)
    else
        Talk(1, "", "B¹n Ch­a §¹t Tíi Mèc Nµy")
    end
end
...
function Moc500()
    ...
    if (GetTask(task_eventxu) >= 600) and (GetTask(task_moc_eventxu) == 0) then
        ...
        tbAwardTemplet:GiveAwardByList(tbAward, "PhÇn Th­ëng Mèc 600")
        SetTask(task_moc_eventxu, 1)
        Msg2SubWorld(
            "<color=green>Chóc mõng cao thñ <color=yellow>" .. GetName() .. "<color> ®· NhËn Thµnh C«ng Mèc Event 600"
        )
        WriteLogPro(
            "dulieu/event_moc1.txt",
            "" ..
                GetAccount() ..
                    "  " ..
                        GetName() ..
                            "\t " ..
                                tonumber(GetLocalDate("%Y%m%d%H%M")) .. "   " .. GetIP() .. "\t Da An max Moc 600\n"
        )
    else
        Talk(1, "", "B¹n Ch­a §¹t Tíi Mèc Nµy")
    end
...
function ResetEvent()
    ...
    if (GetTask(task_eventxu) >= 1000) and (GetTask(MOCRESETEVENT)) == 0 then
        ...
        Talk(
            1,
            "",
            "<color=red>" .. myplayersex() .. " Ch­a Sö Dông §ñ 1000 b¸nh trung thu ®Æc biÖt hoÆc ®· reset 1 lÇn råi"
        )
    end
end
---------
    ...
    if (GetTask(task_eventxu) >= 1200) and (GetTask(task_moc_eventxu) == 1) then
            ...
            "" ..
                GetAccount() ..
                    "  " ..
                        GetName() ..
                            "\t " ..
                                tonumber(GetLocalDate("%Y%m%d%H%M")) .. "   " .. GetIP() .. "\t Da An max Moc 1000\n"
        )
        Msg2SubWorld(
            "<color=green>Chóc mõng cao thñ <color=yellow>" .. GetName() .. "<color> ®· NhËn Thµnh C«ng Mèc Event 1200"
        )
        tbAwardTemplet:GiveAwardByList(tbAward, "PhÇn Th­ëng Mèc 400")
        SetTask(task_moc_eventxu, 2)
        local a = random(1, 5)
        if a == 3 then
            tbAwardTemplet:GiveAwardByList(
                {
                    {
                        szName = "B¶o R­¬ng Tinh S­¬ng Khai",
                        tbProp = {6, 1, 4509, 1, 1},
                        nCount = 1,
                        tbParam = {6, 0, 0, 0, 0, 0}
                    }
        ...
        if c == 5 then
            tbAwardTemplet:GiveAwardByList(
                {
                    {
                        szName = "MÆt N¹ ChiÕn Tr­êng Thanh Gi¶",
                        tbProp = {0, 11, 806, 1, 0, 0},
                        nCount = 1,
                        nExpiredTime = 30 * 24 * 60
                    }
        ...
        if i == 5 then
            tbAwardTemplet:GiveAwardByList(
                {{szName = "Tinh S­¬ng LÖnh", tbProp = {6, 1, 4882, 1, 1}, nCount = 1}},
                "test",
                1
            )
        end
        ...
        if i2 == 1 then
            tbAwardTemplet:GiveAwardByList(
                {{szName = "§å Phæ NguyÖt KhuyÕt", tbProp = {6, 1, random(4429, 4431), 1, 1}, nCount = 1}},
                "test",
                1
            )
        else
            if i2 == 2 then
                ...
                if i2 == 3 then
                    tbAwardTemplet:GiveAwardByList(
                        {{szName = "§å Phæ NguyÖt KhuyÕt", tbProp = {6, 1, random(4436, 4438), 1, 1}, nCount = 1}},
                        "test",
                        1
                    )
                end
