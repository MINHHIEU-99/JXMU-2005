...
enddate=20191101
function main()
	dofile("script/global/g7vn/event/phunuvietnam/hoatai.lua")
	...
	if GetTask(task_hoatai)==0 then
	SetTask(task_hoatai,1)
	end
--	local task_hoatai=4987
	local HienTaiSuDung =GetTask(task_hoatai)
	if (HienTaiSuDung > 1200) then
...
tbAwardTemplet:GiveAwardByList({{nExp_tl = 60e6}}, "test", 1);
	...
	SetTask(task_hoatai,GetTask(task_hoatai)+1)
