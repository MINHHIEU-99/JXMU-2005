...
----------------------------------------
function main()
	dofile("script/global/g7vn/event/quockhanh/eventmain.lua")
	...
	if (nDate >= startdate and nDate <= enddate and nTime >= 0000 and nTime <= 2359) then
		...
		tinsert(tbSay,"NhËn Th­ëng L¸ Cê ChiÕn Th¾ng/NhanMoc")
		tinsert(tbSay,"NhËn Th­ëng Mèc Vßng Hoa ChiÕn Th¾ng./NhanMocBanhThapCam")
	-- tinsert(tbSay,"Mua Hép L¸ Cê ChiÕn Th¾ng/MuaEvent")
...
function NhanMoc()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_eventxu).." <color>L¸ Cê ChiÕn Th¾ng", getn(tbSay), tbSay)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_event_tienvan).." <color>Vßng Hoa ChiÕn Th¾ng", getn(tbSay), tbSay)
...
function Moc600ThapCam()
	...
	if (GetTask(task_event_tienvan) >= 600) and (GetTask(task_moc_event_tienvan) == 0) then
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 600 Vßng Hoa ChiÕn Th¾ng")
...
if CalcFreeItemCellCount() < 20 then
	...
	--		{szName="Ngua Random",tbProp={0,random(3963,3966)},nQuality=1,nCount=1,nExpiredTime=30*24*60},
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event Vßng Hoa ChiÕn Th¾ng 1200")
		...
		tbAwardTemplet:GiveAwardByList(tbAward,"PhÇn Th­ëng Mèc 400") SetTask(task_moc_eventxu,2)
			...
			tbAwardTemplet:GiveAwardByList({{szName = "§å Phæ NguyÖt KhuyÕt KhÝ Giíi",tbProp={6,1,4435,1,1},nCount=1,},}, "test", 1);
				...
				--		tbAwardTemplet:GiveAwardByList({{szName = "B¶o R­¬ng Tinh S­¬ng Yªu §¸i Maxofftion",tbProp={6,1,4971,1,1},nCount=1,tbParam={5,0,0,0,0,0}},}, "test", 1);
					...
					if i6==7 then
					--	tbAwardTemplet:GiveAwardByList({{szName = "B¶o R­¬ng Tinh S­¬ng Béi Maxofftion",tbProp={6,1,4965,1,1},nCount=1,tbParam={9,0,0,0,0,0}},}, "test", 1);
					end;
					if i6==8 then
