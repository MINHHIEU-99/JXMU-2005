...
----------------------------------------
function main()
	dofile("script/global/g7vn/event/noel2020/eventmain.lua")
	...
	if (nDate >= startdate and nDate <= enddate and nTime >= 0000 and nTime <= 2359) then
		...
		tinsert(tbSay,"NhËn Th­ëng ¸o gi¸ng sinh/NhanMoc")
		tinsert(tbSay,"NhËn Th­ëng Mèc Nãn gi¸ng sinh./NhanMocBanhThapCam")
	-- tinsert(tbSay,"Mua Hép ¸o gi¸ng sinh/MuaEvent")
		--tinsert(tbSay,"Reset Event ¸o gi¸ng sinh./ResetEvent")
...
function NhanMoc()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_eventxu).." <color>¸o gi¸ng sinh", getn(tbSay), tbSay)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_event_tienvan).." <color>Nãn gi¸ng sinh", getn(tbSay), tbSay)
...
function Moc600ThapCam()
	...
	if (GetTask(task_event_tienvan) >= 600) and (GetTask(task_moc_event_tienvan) == 0) then
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 600 Nãn gi¸ng sinh")
...
if CalcFreeItemCellCount() < 20 then
	...
	--		{szName="Ngua Random",tbProp={0,random(3963,3966)},nQuality=1,nCount=1,nExpiredTime=30*24*60},
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event Nãn gi¸ng sinh 1200")
...
local Xu = CalcEquiproomItemCount(4,417,1,1)
	if (GetTask(task_eventxu) >= 1000) and (GetTask(MOCRESETEVENT)) ==0 then
		...
		Talk(1,"","<color=red>"..myplayersex().." Ch­a Sö Dông §ñ 1000 ¸o gi¸ng sinh hoÆc ®· reset 1 lÇn råi")
	end
end
---------
----------------------------------------------------Nhan Moc 1000 -----------------------------------------------------------------------------------------------------
...
if CalcFreeItemCellCount() < 20 then
	...
	--	{szName="MËt tÞch Kü N¨ng 150 CÊp 21",tbProp={6,1,4369,1,0,0},nCount=1},			
		...
		--	{szName="MÆt N¹ ChiÕn Tr­êng Vuong Gi¶",tbProp={0,11,806,1,0,0},nCount=1,nExpiredTime=30*24*60},	
	--		{szName="MÆt N¹ ChiÕn Tr­êng ThÇn Gi¶(Kh«ng ThÓ Gia H¹n)",tbProp={0,11,840,1,0,0},nCount=1,nExpiredTime=7*24*60},	
			{szName="MÆt N¹ ChiÕn Tr­êng Thanh Gi¶",tbProp={0,11,806,1,0,0},nCount=1,nExpiredTime=30*24*60},	
		...
		--	tbAwardTemplet:GiveAwardByList({{szName = "§å Phæ NguyÖt KhuyÕt",tbProp={6,1,4434,1,1},nCount=1,},}, "test", 1);--do pho y
					...
					local i4 = random(1,20)
			if i4==5 then
		--	tbAwardTemplet:GiveAwardByList({{szName = "Thiªn S¬n TuyÕt Liªn",tbProp={6,1,1431,1,1},nCount=1,},}, "test", 1);
				end;
