...
----------------------------------------
function main()
	dofile("script/global/g7vn/event/eventthang32021/eventmain.lua")
	...
	--	tinsert(tbSay,"NhËn Th­ëng Mèc Hép quµ may m¾n/NhanMoc")
--		tinsert(tbSay,"NhËn Th­ëng Mèc Tói quµ ®Çu n¨m/NhanMocBanhThapCam")
	-- tinsert(tbSay,"Mua Hép Hép quµ may m¾n/MuaEvent")
		--tinsert(tbSay,"Reset Event Hép quµ may m¾n./ResetEvent")
...
--		if GetAccount()=="boquyx123" or GetAccount()=="luuhuanvlv2019" then
		Say("Chµo Mõng Nh©n SÜ Tham Gia event trung thu cïng VoLamVietPK. B¹n ®· sö dông: <color=red>"..GetTask(task_eventxu).." <color>Hép quµ may m¾n", getn(tbSay), tbSay)
--	end
...
function NhanMoc()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_eventxu).." <color>Hép quµ may m¾n", getn(tbSay), tbSay)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_event_tienvan).." <color>Tói quµ ®Çu n¨m", getn(tbSay), tbSay)
...
function Moc600ThapCam()
	...
	if (GetTask(task_event_tienvan) >= 600) and (GetTask(task_moc_event_tienvan) == 0) then
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 600 Tói quµ ®Çu n¨m")
...
if CalcFreeItemCellCount() < 20 then
	...
	--		{szName="Ngua Random",tbProp={0,random(3963,3966)},nQuality=1,nCount=1,nExpiredTime=30*24*60},
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event Tói quµ ®Çu n¨m 1200")
...
local Xu = CalcEquiproomItemCount(4,417,1,1)
	if (GetTask(task_eventxu) >= 1000) and (GetTask(MOCRESETEVENT)) ==0 then
		...
		Talk(1,"","<color=red>"..myplayersex().." Ch­a Sö Dông §ñ 1000 Hép quµ may m¾n hoÆc ®· reset 1 lÇn råi")
	end
end
---------
----------------------------------------------------Nhan Moc 1000 -----------------------------------------------------------------------------------------------------
