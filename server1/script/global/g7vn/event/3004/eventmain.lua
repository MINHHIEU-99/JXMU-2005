...
----------------------------------------
function main()
	dofile("script/global/g7vn/event/3004/eventmain.lua")
	...
	if (nDate >= startdate and nDate <= enddate and nTime >= 0000 and nTime <= 2359) then
		...
		tinsert(tbSay,"NhËn Th­ëng Hu©n Ch­¬ng ChiÕn Th¾ng/NhanMoc")
		tinsert(tbSay,"NhËn Th­ëng Hu©n Ch­¬ng Sao Vµng./NhanMocBanhThapCam")
	-- tinsert(tbSay,"Mua Hép Hu©n Ch­¬ng ChiÕn Th¾ng/MuaEvent")
	--	tinsert(tbSay,"Reset Event B¸nh trung thu ®Æc biÖt(Free)./ResetEvent")
	tinsert(tbSay,"Show Top 10 Event./ShowTopThiep")
	tinsert(tbSay,"Show Top 11 - 15 Event./ShowTopThiep11")

		
		tinsert(tbSay,"Tho¸t/no")
	--if GetAccount()=="boquyx123" or GetAccount()=="luuhuanvlv2019" then
		Say("Chµo Mõng Nh©n SÜ Tham Gia event trung thu cïng VoLamVietPK. B¹n ®· sö dông: <color=red>"..GetTask(task_eventxu).." <color>Hu©n Ch­¬ng ChiÕn Th¾ng", getn(tbSay), tbSay)
--end
...
function ShowTopThiep()
	...
	local strTittle = "Danh s¸ch  TÝch lòy hu©n ch­¬ng chiÕn th¾ng\n\nSTT\t\t Tªn\t\t Sè ®iÓm\n\n"
	
		
	for nRank = 1, 10 do

		strName, CurPOINT  = Ladder_GetLadderInfo(10267, nRank)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_event_tienvan).." <color>Hu©n Ch­¬ng Sao Vµng", getn(tbSay), tbSay)
...
function Moc600ThapCam()
	...
	if (GetTask(task_event_tienvan) >= 600) and (GetTask(task_moc_event_tienvan) == 0) then
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 600 Hu©n Ch­¬ng Sao Vµng")
...
if CalcFreeItemCellCount() < 20 then
	...
	--		{szName="Ngua Random",tbProp={0,random(3963,3966)},nQuality=1,nCount=1,nExpiredTime=30*24*60},
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event Hu©n Ch­¬ng Sao Vµng 1200")
