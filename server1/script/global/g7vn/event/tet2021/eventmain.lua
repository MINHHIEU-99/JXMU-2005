...
----------------------------------------
function main()
	dofile("script/global/g7vn/event/tet2021/eventmain.lua")
	...
	if (nDate >= startdate and nDate <= enddate and nTime >= 0000 and nTime <= 2359) then
		...
		tinsert(tbSay,"NhËn Th­ëng Mèc Ph¸o Hoa/NhanMoc")
		tinsert(tbSay,"NhËn Th­ëng Mèc R­îu Vang/NhanMocBanhThapCam")
	-- tinsert(tbSay,"Mua Hép Ph¸o Hoa/MuaEvent")
		--tinsert(tbSay,"Reset Event Ph¸o Hoa./ResetEvent")
	tinsert(tbSay,"Show Top Event./ShowTopThiep")
			 tinsert(tbSay,"GhÐp ThiÖp Chóc TÕt/GhepThiepGiangSinh")
...
function GhepThiepGiangSinh()
	...
	local tbFormulaList = 
	{
		   ...
		   [1] = 
		{
			tbMaterial = 
			{
				  {szName = "ThiÖp Chóc TÕt 1", tbProp = {6,1,1328,1,1},nCount = 1},
				  {szName = "ThiÖp Chóc TÕt 2", tbProp = {6,1,1329,1,1},nCount = 1},
				  {szName = "ThiÖp Chóc TÕt 3", tbProp = {6,1,1330,1,1},nCount = 1},
				  {szName = "ThiÖp Chóc TÕt 4", tbProp = {6,1,1331,1,1},nCount = 1},
				  {szName = "ThiÖp Chóc TÕt 5", tbProp = {6,1,1332,1,1},nCount = 1},
               ...
               [2] = 
		{
			tbMaterial = 
			{
				  {szName = "ThiÖp Chóc TÕt 1", tbProp = {6,1,1328,1,1},nCount = 5},
				  {szName = "ThiÖp Chóc TÕt 2", tbProp = {6,1,1329,1,1},nCount = 5},
				  {szName = "ThiÖp Chóc TÕt 3", tbProp = {6,1,1330,1,1},nCount = 5},
				  {szName = "ThiÖp Chóc TÕt 4", tbProp = {6,1,1331,1,1},nCount = 5},
				  {szName = "ThiÖp Chóc TÕt 5", tbProp = {6,1,1332,1,1},nCount = 5},
...
function NhanMoc()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_eventxu).." <color>Ph¸o Hoa", getn(tbSay), tbSay)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_event_tienvan).." <color>R­îu Vang", getn(tbSay), tbSay)
...
function Moc600ThapCam()
	...
	if (GetTask(task_event_tienvan) >= 600) and (GetTask(task_moc_event_tienvan) == 0) then
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 600 R­îu Vang")
...
if CalcFreeItemCellCount() < 20 then
	...
	--		{szName="Ngua Random",tbProp={0,random(3963,3966)},nQuality=1,nCount=1,nExpiredTime=30*24*60},
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event R­îu Vang 1200")
...
local Xu = CalcEquiproomItemCount(4,417,1,1)
	if (GetTask(task_eventxu) >= 1000) and (GetTask(MOCRESETEVENT)) ==0 then
		...
		Talk(1,"","<color=red>"..myplayersex().." Ch­a Sö Dông §ñ 1000 Ph¸o Hoa hoÆc ®· reset 1 lÇn råi")
	end
end
---------
----------------------------------------------------Nhan Moc 1000 -----------------------------------------------------------------------------------------------------
