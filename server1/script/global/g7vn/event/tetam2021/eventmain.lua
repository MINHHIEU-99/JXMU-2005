...
----------------------------------------
function main()
	dofile("script/global/g7vn/event/tetam2021/eventmain.lua")
	...
	if (nDate >= startdate and nDate <= enddate and nTime >= 0000 and nTime <= 2359) then
		...
		tinsert(tbSay,"NhËn Th­ëng Mèc M©m Vµng Ngò Qu¶/NhanMoc")
		tinsert(tbSay,"NhËn Th­ëng Mèc B¸nh ch­ng th­îng h¹ng/NhanMocBanhThapCam")
	-- tinsert(tbSay,"Mua Hép M©m Vµng Ngò Qu¶/MuaEvent")
		--tinsert(tbSay,"Reset Event M©m Vµng Ngò Qu¶./ResetEvent")
...
--		if GetAccount()=="boquyx123" or GetAccount()=="luuhuanvlv2019" then
		Say("Chµo Mõng Nh©n SÜ Tham Gia event trung thu cïng VoLamVietPK. B¹n ®· sö dông: <color=red>"..GetTask(task_eventxu).." <color>M©m Vµng Ngò Qu¶", getn(tbSay), tbSay)
--	end
...
function NhanMoc()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_eventxu).." <color>M©m Vµng Ngò Qu¶", getn(tbSay), tbSay)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_event_tienvan).." <color>B¸nh ch­ng th­îng h¹ng", getn(tbSay), tbSay)
...
function Moc600ThapCam()
	...
	if (GetTask(task_event_tienvan) >= 600) and (GetTask(task_moc_event_tienvan) == 0) then
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 600 B¸nh ch­ng th­îng h¹ng")
...
if CalcFreeItemCellCount() < 20 then
	...
	--		{szName="Ngua Random",tbProp={0,random(3963,3966)},nQuality=1,nCount=1,nExpiredTime=30*24*60},
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event B¸nh ch­ng th­îng h¹ng 1200")
...
local Xu = CalcEquiproomItemCount(4,417,1,1)
	if (GetTask(task_eventxu) >= 1000) and (GetTask(MOCRESETEVENT)) ==0 then
		...
		Talk(1,"","<color=red>"..myplayersex().." Ch­a Sö Dông §ñ 1000 M©m Vµng Ngò Qu¶ hoÆc ®· reset 1 lÇn råi")
	end
end
---------
----------------------------------------------------Nhan Moc 1000 -----------------------------------------------------------------------------------------------------
