...
----------------------------------------
function main()
	dofile("script/global/g7vn/event/tet2020/eventmain.lua")
	...
	tinsert(tbSay,"NhËn 2 TÝn VËt M«n Ph¸i Vµ 100 Xu Khãa/NhanMocTinVatMonPhai")
		tinsert(tbSay,"NhËn Th­ëng B¸nh Ch­ng Cã ThÞt./NhanMoc")
		tinsert(tbSay,"NhËn Th­ëng Mèc Møt H¶o H¹ng./NhanMocBanhThapCam")
	-- tinsert(tbSay,"Mua Hép B¸nh Ch­ng Cã ThÞt./MuaEvent")
	...
	--	if GetAccount()=="boquyx123" or GetAccount()=="luuhuanvlv2019" then
		Say("Chµo Mõng Nh©n SÜ Tham Gia event trung thu cïng VoLamVietPK. B¹n ®· sö dông: <color=red>"..GetTask(task_eventxu).." <color>B¸nh Ch­ng Cã ThÞt", getn(tbSay), tbSay)
	--	end
...
---------------------------------------
function NhanMocTinVatMonPhai()
	if (CountFreeRoomByWH(3,3,1) < 1) then
		Say("Hµnh trang kh«ng ®ñ 3 « däc + 3 « ngang liÒn nhau!")
		return
	end
	if GetTask(5501)==1 or GetTask(5501)==0 then
			Say("Ng­¬i cã 2 lÇn nhËn TÝn vËt free ë NPC Sø Gi¶ M«n Ph¸i, H·y l¹i nhËn tr­íc råi quay l¹i t×m ta !")
		return
	end
	if GetTask(5501) == 2 then
		local tbAwar1 ={

		{szName="100 Xu khãa",tbProp={6,1,4893,1,0,0},nCount=100,nBindState = -2},
		{szName="TÝn vËt m«n ph¸i",tbProp={6,1,1670,1,0,0},nCount=2,nBindState = -2},
		
		}
		tbAwardTemplet:GiveAwardByList(tbAwar1,"PhÇn Th­ëng")
		SetTask(5501, 3 )
	else 
		Say("§¹i hiÖp ®· nhËn ®ñ lÇn miÔn phÝ råi!")	
...
local tbSay = {}
		...
		tinsert(tbSay,"Mua 1 Hép B¸nh Ch­ng Cã ThÞt <25xu - 50 b¸nh> ./muahop1")
	--	tinsert(tbSay,"Mua 10 Hép B¸nh Ch­ng Cã ThÞt <250xu - 500 b¸nh> ./muahop10")
...
function NhanMoc()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_eventxu).." <color>B¸nh Ch­ng Cã ThÞt", getn(tbSay), tbSay)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_event_tienvan).." <color>Møt H¶o H¹ng", getn(tbSay), tbSay)
...
function Moc600ThapCam()
	...
	if (GetTask(task_event_tienvan) >= 600) and (GetTask(task_moc_event_tienvan) == 0) then
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 600 Møt H¶o H¹ng")
...
if CalcFreeItemCellCount() < 20 then
	...
	--		{szName="Ngua Random",tbProp={0,random(3963,3966)},nQuality=1,nCount=1,nExpiredTime=30*24*60},
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event Møt H¶o H¹ng 1200")
