...
function main()
	...
	if (nDate >= startdate and nDate <= enddate and nTime >= 0000 and nTime <= 2359) then
		...
		tinsert(tbSay,"NhËn Th­ëng Mèc "..teneventxu.."/NhanMoc")
		tinsert(tbSay,"NhËn Th­ëng Mèc "..teneventtienvan.."/NhanMocBanhThapCam")
	         tinsert(tbSay,"Mua Hép "..teneventxu.."/MuaEvent")
		--tinsert(tbSay,"Reset Event "..teneventxu.."./ResetEvent")
	          tinsert(tbSay,"Show Top Event./ShowTopThiep")
	        tinsert(tbSay,"GhÐp ThiÖp Gi¸ng Sinh/GhepThiepGiangSinh")
		tinsert(tbSay,"Tho¸t/no")

		Say("Chµo Mõng Nh©n SÜ Tham Gia event Gi¸ng Sinh cïng VoLamHKMP. B¹n ®· sö dông: <color=red>"..GetTask(task_eventxu).." <color>"..teneventxu.."", getn(tbSay), tbSay)
...
function GhepThiepGiangSinh()
	...
	local tbFormulaList = 
	{
		   ...
		   [1] = 
		{
			tbMaterial = 
			{
				  {szName = "ThiÖp gi¸ng sinh 1", tbProp = {6,1,1328,1,1},nCount = 1},
				  {szName = "ThiÖp gi¸ng sinh 2", tbProp = {6,1,1329,1,1},nCount = 1},
				  {szName = "ThiÖp gi¸ng sinh 3", tbProp = {6,1,1330,1,1},nCount = 1},
				  {szName = "ThiÖp gi¸ng sinh 4", tbProp = {6,1,1331,1,1},nCount = 1},
				  {szName = "ThiÖp gi¸ng sinh 5", tbProp = {6,1,1332,1,1},nCount = 1},
               ...
               [2] = 
		{
			tbMaterial = 
			{
				  {szName = "ThiÖp gi¸ng sinh 1", tbProp = {6,1,1328,1,1},nCount = 5},
				  {szName = "ThiÖp gi¸ng sinh 2", tbProp = {6,1,1329,1,1},nCount = 5},
				  {szName = "ThiÖp gi¸ng sinh 3", tbProp = {6,1,1330,1,1},nCount = 5},
				  {szName = "ThiÖp gi¸ng sinh 4", tbProp = {6,1,1331,1,1},nCount = 5},
				  {szName = "ThiÖp gi¸ng sinh 5", tbProp = {6,1,1332,1,1},nCount = 5},

			},
			tbProduct = {szName="MÆt N¹ ChiÕn Tr­êng ThÇn Gi¶(Kh«ng ThÓ Gia H¹n)", tbProp={0,11,840,1,0,0},nExpiredTime=15*24*60},
			nWidth = 1,
			nHeight = 1,
			nFreeItemCellLimit = 0.04
		},	
               ...
               [4] = 
		{
			...
			tbProduct = {szName="Ngùa H·n HuyÕt Long C©u(3 ngµy)", tbProp={0, 6049}, nQuality = 1,nExpiredTime=15*24*60},
			nWidth = 1,
			nHeight = 1,
			nFreeItemCellLimit = 0.04
		},	
...
local tbSay = {}
		...
		tinsert(tbSay,"Mua 1 Hép Lä B¸nh <200xu - 50 b¸nh> ./muahop1")
		tinsert(tbSay,"Tho¸t/no")
...
function muahop1()
local tbItem = {
	{szName="Hép b¸nh trung thu", tbProp={6,1,4950,1,0,0},nCount = 1,nExpiredTime=20200116},
}
	...
	if  nCount_muaban >= 200  then
		...
		Say("Kh«ng ®ñ 200 Xu, Xin Xem L¹i")
	end
...
function NhanMoc()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_eventxu).." <color>"..teneventxu.."", getn(tbSay), tbSay)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_event_tienvan).." <color>"..teneventtienvan.."", getn(tbSay), tbSay)
...
function Moc600ThapCam()
	...
	if (GetTask(task_event_tienvan) >= 600) and (GetTask(task_moc_event_tienvan) == 0) then
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 600 "..teneventtienvan.."")
...
if CalcFreeItemCellCount() < 20 then
	...
	--		{szName="Ngua Random",tbProp={0,random(3963,3966)},nQuality=1,nCount=1,nExpiredTime=30*24*60},
		...
		{szName="NguyÖt KhuyÕt Tinh Th¹ch",tbProp={6,1,4817,1,0,0},nCount=2},			
			},
		}	
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event "..teneventtienvan.." 1200")
...
local Xu = CalcEquiproomItemCount(4,417,1,1)
	if (GetTask(task_eventxu) >= 1000) and (GetTask(MOCRESETEVENT)) ==0 then
		...
		Talk(1,"","<color=red>"..myplayersex().." Ch­a Sö Dông §ñ 1000 "..teneventxu.." hoÆc ®· reset 1 lÇn råi")
	end
end
---------
----------------------------------------------------Nhan Moc 1000 -----------------------------------------------------------------------------------------------------
...
if CalcFreeItemCellCount() < 20 then
	...
	--		{szName="LÔ bao HuyÕt ChiÕn (§¹i)",tbProp={6,1,4898,1,0,0},nCount=2},	
			{szName="B¸ch LuyÖn Thµnh C­¬ng",tbProp={6,1,4906,1,0,0},nCount=2},			
			{szName="Ba l« Con Cãc",tbProp={6,1,2308,1,0,0},nCount=1,nExpiredTime=TimeHetHanEventFree},			
