...
----------------------------------------
function main()
	dofile("script/global/g7vn/event/1402/eventmain.lua")
	...
	if (nDate >= startdate and nDate <= enddate and nTime >= 0000 and nTime <= 2359) then
		...
		tinsert(tbSay,"NhËn Th­ëng NhÉn Kim C­¬ng./NhanMoc")
		tinsert(tbSay,"NhËn Th­ëng Mèc Thái Son./NhanMocBanhThapCam")
	-- tinsert(tbSay,"Mua Hép NhÉn Kim C­¬ng./MuaEvent")
...
--	tinsert(tbSay,"Show Top Event./ShowTopThiep")
	...
	--	if GetAccount()=="boquyx123" or GetAccount()=="luuhuanvlv2019" then
		Say("Chµo Mõng Nh©n SÜ Tham Gia event trung thu cïng VoLamVietPK. B¹n ®· sö dông: <color=red>"..GetTask(task_eventxu).." <color>NhÉn Kim C­¬ng", getn(tbSay), tbSay)
	--	end
...
local tbSay = {}
		...
		tinsert(tbSay,"Mua 1 Hép NhÉn Kim C­¬ng <25xu - 50 b¸nh> ./muahop1")
	--	tinsert(tbSay,"Mua 10 Hép NhÉn Kim C­¬ng <250xu - 500 b¸nh> ./muahop10")
...
function NhanMoc()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_eventxu).." <color>NhÉn Kim C­¬ng", getn(tbSay), tbSay)
...
function NhanMocBanhThapCam()
	local tbSay = {}
		...
		tinsert(tbSay,"Tho¸t/no")
		Say(" HiÖn B¹n §ang Sö Dông <color=green>"..GetTask(task_event_tienvan).." <color>Thái Son", getn(tbSay), tbSay)
...
function Moc600ThapCam()
	...
	if (GetTask(task_event_tienvan) >= 600) and (GetTask(task_moc_event_tienvan) == 0) then
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event 600 Thái Son")
...
if CalcFreeItemCellCount() < 20 then
	...
	--		{szName="Ngua Random",tbProp={0,random(3963,3966)},nQuality=1,nCount=1,nExpiredTime=30*24*60},
		...
		Msg2SubWorld("<color=green>Chóc mõng cao thñ <color=yellow>"..GetName().."<color> ®· NhËn Thµnh C«ng Mèc Event Thái Son 1200")
