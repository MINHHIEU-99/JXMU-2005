...
dofile("script/global/quanlygame/item/trungthanh.lua")
...
function upgrade_skilllevel(n_skillid)
	if (HaveMagic(n_skillid) == -1 or HaveMagic(n_skillid) >= 16) then
			Talk(1, "", "QuyÓn nµy chØ cã thÓ gióp nhµ ng­¬i ®¹t ng­ìng skill tíi cÊp 16.hiÖn t¹i nhµ ng­êi ®· v­ît råi xin h·y chän quyÓn kh¸c.");
		return	
	end
	
	if (ConsumeItem(3, 1, 6, 1, 4544, -1) == 1) then
		AddMagic(n_skillid, 16);
