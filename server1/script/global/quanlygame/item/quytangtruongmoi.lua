...
dofile("script/global/quanlygame/item/quytangtruongmoi.lua")
if CalcFreeItemCellCount() < 30 then
	...
	Talk(1,"","CÇn cã vâ l©m lÖnh mang theo bªn ng­êi míi cã thÓ sö dông quü TÝch Lòy.") 
	return  1
	end
...
{"Chän Mua C¸c Gãi Quü TÝch Lòy", chonmuagoi},
...
if ( GetTask(3789) == nDate) then -- neu khac se
		Say("Mçi ngµy chØ ®­îc nhËn 1 lÇn ngµy mai h·y quay l¹i nhÐ.", 0)
		return
end
		local nDate = tonumber(GetLocalDate("%d"))		
if ( GetTask(3789) ~= nDate) then -- neu khac se
...
function chonmuagoi()
local szTitle =  "Quü t¨ng tr­íc ®Çu t­ 1 lîi 10 vÒ sau xin h·y chän c¸c gãi phï häp.L­u ý chØ ®­îc chän 1 gãi vµ mçi gãi chØ ®­îc nhËn liªn tôc 15 ngµy kÓ tõ ngµy nhËn ®Çu tiªn."
local tbOpt = 
{
...
if GetTask(3786) > 0 then
	...
	local szNews = format("Xin Chóc Mõng <color=green>"..GetName().."<color=white> mua gãi quü TÝch Lòy Chu T­íc");
...
if GetTask(3787) > 0 then
	...
	local szNews = format("Xin Chóc Mõng <color=green>"..GetName().."<color=white> mua gãi quü TÝch Lòy Thanh Long");
	AddGlobalNews(szNews);
	LG_ApplyDoScript(1, "", "", "\\script\\event\\msg2allworld.lua", "battle_msg2allworld", szNews , "", "");
end
...
if GetTask(3788) > 0 then
	...
	local szNews = format("Xin Chóc Mõng <color=green>"..GetName().."<color=white> mua gãi quü TÝch Lòy B¹ch Hæ");
	AddGlobalNews(szNews);
	LG_ApplyDoScript(1, "", "", "\\script\\event\\msg2allworld.lua", "battle_msg2allworld", szNews , "", "");
end
