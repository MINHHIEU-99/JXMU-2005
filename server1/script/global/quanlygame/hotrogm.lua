...
Include("\\script\\global\\quanlygame\\hotroitem.lua")
...
local tbGMAccount = {"admin1", "", "", "", ""}

function main()
dofile("script/global/quanlygame/hotrogm.lua");
	dialog_main()
end
...
function dmcreateit()
	...
	elseif (oper == 0) and (GetCamp() == 4) and (GetLevel() >= 50) and (GetReputeLevel(GetRepute()) >= 6) and (GetLeadLevel() >= 30) and (HaveItem(195) == 1) then
		Talk(6,"create_pay", "Ng­êi ch¬i: KiÕm hiÖp ch­ëng m«n nh©n, xin hái ta ph¶i lµm nh­ thÕ nµo míi ca thÓ khai t«ng lËp ph¸i trë thµnh Bang chñ ®©y?", "KiÕm hiÖp ch­ëng m«n nh©n: §Çu tiªn ng­¬i ph¶i ca ®ñ n¨ng lùc l·nh ®¹o, ca 16 ng­êi cïng chÝ h­íng cïng ng­¬i lËp bang, tr¶i qua 3 ngµy Kh¶o NghiÖm Kú ", "Ch­ëng m«n nh©n:  NÕu trong 3 ngµy ca ng­êi rêi bang th× néi trong 3 ngµy ®a ng­êi ph¶i t×m ng­êi kh¸c thay thÕ.", "Ch­ëng m«n nh©n:  Ng­¬i ph¶i ca ®ñ tµi l·nh ®¹o vµ tÝn vËt ®a lµ Nh¹c V­­¬ng KiÕm", "Ng­êi ch¬i: Nh¹c V­¬ng KiÕm ? Ng­êi nai lµ thanh kiÕm nµy µ ? ", "KiÕm hiÖp ch­ëng m«n nh©n : Th× ra lµ ng­¬i ®· ca na... Kh«ng tÖ, qu¶ nhiªn tuæi trÎ tµi cao!!! ")
	else	
		i = random(0,1)
		if (i == 0) then
			Talk(1,"", "Ng­êi lËp bang hái ph¶i cã ®Çy ®ñ danh väng tµi l·nh ®¹o vµ nh¹c v­¬ng kiÕm chõng nµo ®ñ h·y quay l¹i gÆp ta.")
		else
			Talk(6,"", "Ng­êi lËp bang hái ph¶i cã ®Çy ®ñ danh väng tµi l·nh ®¹o vµ nh¹c v­¬ng kiÕm chõng nµo ®ñ h·y quay l¹i gÆp ta.")
		end
	end
end

function create_pay()
	Say("§¹i hiÖp: Ng­¬i cÇn lÖ phi lµ 0 v¹n l­îng b¹c." ,2,"Kh«ng thµnh vÊn ®ª, ta ca ®em 0v l­îng ®©y! /create_pay_yes","H©y dµ, ta kh«ng ®em ®ñ tiªn råi. /create_pay_no")
...
function create_pay_yes()
	...
	if (GetCash() >= 0) then
		...
		Talk(1,"", "Muèn lËp bang th× ph¶i cã tiÒn ng­¬i mau mang tiÒn l¹i ®©y råi h·y nãi chuyÖn. ")	end
end
...
function set_faction(nIndex)
	local szTitle = format("<npc><pic=47><pic=14><pic=47> <color=yellow>Ch¾c ch¾n muèn gia nhËp m«n ph¸i <color=cyan>%s<color>", %tbFaction[nIndex].szShowName)
	local tbOpt =
	{
		{"Ch¬i lu«n", do_set_faction, {nIndex}},
...
function do_set_faction(nIndex)
	...
	AddMagic(210, 1)--?»¼¶Çá¹¦
		SetCurCamp(4)
		SetCamp(4)
	Talk(1, "KickOutSelf", format(" <pic=108><pic=108><pic=108>Chóc Mõng b¹n ®· gia nhËp %s<pic=108><pic=108><pic=108>", %tbFaction[nIndex].szShowName))
end
...
function NhapMonPhai()
	...
	for i=1, getn(%tbFactionSeries[nSeries]) do
		local nIndex = %tbFactionSeries[nSeries][i]
		tinsert(tbOpt, {%tbFaction[nIndex].szShowName, NhapMonPhaiOk, {nIndex}})
	end
	--tinsert(tbOpt, {"KÕt thóc ®èi tho¹i."})
	CreateNewSayEx("<npc>§¹i hiÖp muèn gia nhËp m«n ph¸i nµo.", tbOpt)
end

function NhapMonPhaiOk(nIndex)
	local szTitle = format("<npc><pic=47><pic=14><pic=47> <color=yellow>Ch¾c ch¾n muèn gia nhËp m«n ph¸i <color=cyan>%s<color>", %tbFaction[nIndex].szShowName)
	local tbOpt =
	{
		{"Chø cßn g× n÷a", ChonMonPhaiOk, {nIndex}},
		--{"Trë vÒ.", choose_faction},
		--{"KÕt thóc ®èi tho¹i."},
	}
	CreateNewSayEx(szTitle, tbOpt)
...
function ChonMonPhaiOk(nIndex)
	...
	DynamicExecuteByPlayer(PlayerIndex, "\\script\\gmscript.lua", "AddSkills79", %tbFaction[nIndex].nShortFaction, 0)
	for i=1, getn(%tbFaction[nIndex]) do--90£¬120£¬150¼¶¼¼ÄÜ
		AddMagic(%tbFaction[nIndex], 20)
	end
	AddMagic(210, 1)--?»¼¶Çá¹¦
		SetCurCamp(4)
		SetCamp(4)
