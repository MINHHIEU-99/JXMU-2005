...
Include("\\script\\global\\mrt\\hotrotanthu\\vongsangtanthu.lua")
Include("\\script\\global\\mrt\\hotrotanthu\\giaiketnhanvat.lua")
Include("\\script\\global\\mrt\\hotrotanthu\\nhanskillhotro.lua")
...
Include("\\script\\global\\mrt\\thunghiem\\trangbihoangkimmaxop.lua")
...
Include("\\script\\global\\mrt\\thunghiem\\trangbihoangkimmaxopkhoa.lua")
Include("\\script\\global\\mrt\\testserver.lua")
Include("\\script\\global\\titlefuncs.lua")
Include("\\script\\global\\systemconfig.lua");
Include("\\script\\dailogsys\\dailogsay.lua");
Include("\\script\\global\\mrt\\configserver\\configall.lua")
Include("\\script\\global\\mrt\\hotrotanthu\\phanthuonghotro.lua")
-----------------------------------------------------------------------tbFaction------------------------------------------------------------------------
local tbFaction =
{
	[1] =
...
function main(nItemIndex)
	dofile("script/global/general/lenhbaitanthu.lua")
		...
		local tbSay = {format(TITLEDIALOG, GetName(), GetAccount(),day,mm,year,hour,mmin ,nW,nX,nY)};
			if HoTroTestGame == 1 then -- kh«ng cÇn cmt dßng mµ chØ cÇn chØnh HoTroTestGame vÒ 0 lµ t¾t ( ®Ó ý cho c¸c phÇn sau )
...
--			tinsert(tbSay, "lÊy LÖnh bµi/LBAdmin");
			end
			if NhanHoTroKyNang1xDen6x ==1 and GetTask(5744) == 1 then
			tinsert(tbSay, "Xãa toµn bé item trong hµnh trang/xoatoanboitem");
			tinsert(tbSay, "NhËn hæ trî skill 10 ®Õn 90/HoTroSkill2");
			end
			if ChinhServerPkNhanFullDoVaCap == 1 then
			tinsert(tbSay, "NhËn hæ trî/nhanhotropk");
			end
	--		tinsert(tbSay, "Hæ trî theo cÊp ®é/PhanThuongHoTro");
			tinsert(tbSay, "Xãa toµn bé item trong hµnh trang/xoatoanboitem");
			tinsert(tbSay, "Thay ®æi danh hiÖu/change_title");
			tinsert(tbSay, "Gi¶i kÑt nh©n vËt/KetAcc");
			tinsert(tbSay,"Häc L¹i Khinh C«ng.", AddMagic(210,1) )
...
ItemIndex4 = AddItem(6,1,4906,0,0,0)
ItemIndex5 = AddItem(6,1,4890,0,0,0)
...
function xoatoanboitem()
    Say(
        "<color=green>L­u ý:<color> <color=yellow>Muèn Xãa R­¬ng Hµnh Trang CÇn Ph¶i KiÓm Tra\nTrang BÞ VËt PhÈm Tr­íc Khi Xãa.\n<color=green>Lo¹i Trõ 6 VËt PhÈm HÖ Thèng Gi÷ L¹i:<color>\n<color=red>\t* Giang Hå LÖnh, ThÇn Hµnh Phï, Håi Thµnh Phï\n\t\t\tKim Nguyªn B¶o, TiÒn Xu, Tói M¸u T©n Thñ.<color>",
        2,
        "X¸c NhËn Muèn Xãa/ClearBag",
        "Hñy Bá/No"
    )
end
function ClearBag()
    ...
    for i = 1, getn(tbRoomItems) do
        local nItemIdx = tbRoomItems[i]
        local szName = GetItemName(nItemIdx)

        -- KiÓm tra nÕu vËt phÈm kh«ng bÞ khãa (-2 lµ tr¹ng th¸i bÞ khãa)
        if GetItemBindState(nItemIdx) ~= -2 then
            -- Thùc hiÖn xãa chØ ®èi víi vËt phÈm kh«ng bÞ khãa
...
function LBAdmin()
	local index = AddItem(6,1,4906,0,0,0,0); SetItemBindState(index,-2)
	local index = AddItem(6,1,4890,0,0,0,0); SetItemBindState(index,-2)
