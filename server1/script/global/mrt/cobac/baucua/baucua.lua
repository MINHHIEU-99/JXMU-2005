...
-------------------------------------------------------------Luu Dat Cuoc Chung Bau Cua --------------------------------------------------------
		local LogOpenItemFileName = openfile("script/global/mrt/cobac/baucua/logs/baucua_"..date("%d_%m_%Y")..".log", "a");
...
dofile("script/global/mrt/cobac/baucua/baucua.lua")
...
local nNowTime = tonumber(date( "%H%M "))
    if (nNowTime > ThoiGianBatDauBauCua and nNowTime < ThoiGianKetThucBauCua ) then   
		DynamicExecute("\\script\\global\\mrt\\cobac\\baucua\\baucua.lua", "FishGame:Init")
    local tab_Chat = {
    ...
    SetNpcScript(nNpcIndex,"\\script\\global\\mrt\\cobac\\baucua\\baucua.lua") 
else
    local tab_Chat2 = {
      "     <pic=35><color=green><enter>"..ThoiGianBatDauBauCuaText.."<color>",
		...
		local ranTimer = random(10,20)
		SetNpcTimer(nNpcIndex,ranTimer*18)
		DynamicExecute("\\script\\global\\mrt\\cobac\\baucua\\baucua.lua", "FishGame:Stop")
    end
end
...
function Add_Npc_BauCua()
    ...
    for i=1,getn(tb_npc_hotro) do
            ...
            SetNpcScript(npcdialog,"\\script\\global\\mrt\\cobac\\baucua\\baucua.lua")     
    end
end
