...
Include("\\script\\task\\newtask\\newtask_head.lua") -- Task hoµn Thµnh Mçi 100 lÇn
...
function checkTask_Limit()
    ...
    if nNowDate ~= nOldDate then
        ...
        if nNum >= MISSION_OF_DAY then
            Say("Mçi ngµy kh«ng thÓ lµm qu¸ " .. MISSION_OF_DAY .. " nhiÖm vô ", 0)
            return 0
        end
...
function Task_NewVersionAward()
    ...
    G_ACTIVITY:OnMessage("FinishYesou", nNum, nCancelNum) -- dïng cho chÕ ®é config cña AcTivitiSYS
	tbAward = {
		[10] = {szName = "§iÓm Kinh NghiÖm", nExp = 60000},
		[20] = {szName = "§iÓm Kinh NghiÖm", nExp = 60000},
		[30] = {szName = "§iÓm Kinh NghiÖm", nExp = 60000},
		[40] = {szName = "§iÓm Kinh NghiÖm", nExp = 60000},
	}
    ...
    -- if (nNum - nCancelNum) == 10 then -- ( sè lÇn hoµn thµnh - sè lÇn hñy bá ) == 10 th× ...
	local nCompletedNum = nNum - nCancelNum
		for _, targetNum  in solanhoanthanh do
			if nCompletedNum == targetNum then
				...
				Msg2SubWorld(
					format("Chóc mõng ®¹i hiÖp <color=green>%s<color> hoµn thµnh %d nhiÖm vô d· tÈu trong ngµy <color> nhËn ®­îc  <color=yellow>%s <color>",
...
function tasklink_entence()
    ...
    if (tl_gettaskcourse() == 0) then
        Say(
            " VŞ " ..
                GetPlayerSex() ..
                    " nµy xem ra ®· b«n ba giang hå ®­îc mét thêi gian dµi råi nhØ, cã muèn tham gia kh¶o nghiÖm nhiÖm vô liªn tôc cña ta kh«ng?",
            3,
    ...
    if (tl_gettaskcourse() == 3) then
        ...
        Say(
            ":<enter>VŞ " ..
                GetPlayerSex() ..
                    " ®· hoµn thµnh <color=yellow>" ..
                        myCountTimes ..
                            "<color>, nhiÖm vô nµy ngµy h«m nay cã thÓ thùc hiÖn l¹i<color=yellow>" ..
                                (MISSION_OF_DAY - GetTask(ID_TASKLINK_LIMITNUM)) .. "<color>, ng­¬i cã muèn lµm n÷a kh«ng?",
            2,
            "§­¬ng nhiªn, mau cho ta biÕt nhiÖm vô tiÕp theo lµ g× /Task_TaskProcess",
            "§Ó ta nghØ ng¬i mét l¸t ®·! Ta bËn råi/Task_Wait"
        )
...
function Task_MainDialog()
    ...
    if myTaskTotalNum == 0 or myTaskTotalNum == nil then
        ...
        myTitleText =
            "D· TÈu:<enter><enter> §©y lµ <color=green>" .. myTaskTotalNum .. "<color> nhiÖm vô thø, " .. myTaskInfo
    end
    ...
    Say(
        ...
        "Ta muèn dïng 1xu ®Ó hoµn thµnh nhiÖm vô!/hoanhthanhdatau1xu",
...
function Task_CancelConfirm()
    ...
    if (myTaskCancel == 0) then
        Say(
            " HiÖn t¹i b¹n kh«ng cã c¬ héi nµo ®Ó hñy bá nhiÖm vô, b¹n chØ cã thÓ lµm l¹i tõ ®Çu th«i! §ång thêi phÇn th­ëng tİch lòy cña b¹n lóc tr­íc sÏ ®­îc tİnh l¹i tõ ®Çu.",
            3,
        ...
        Say(
            " HiÖn t¹i b¹n cßn" ..
                myTaskCancel .. " sè lÇn c¬ héi hñy bá nhiÖm vô, b¹n x¸c ®Şnh hñy bá nhiÖm vô lÇn nµy ®óng kh«ng?",
            2,
...
function Task_NormalCancel()
    Say(
        " B¹n suy nghÜ kü hñy bá nhiÖm vô lÇn nµy ®óng kh«ng?",
        2,
...
function hoanhthanhdatau1xu()
    if ConsumeEquiproomItem(1, 4, 417, 1, -1) == 1 then
        ...
        Say("D· TÈu: Kh«ng lµm mµ ®ßi cã ¨n sao? Mau ®i lµm nhiÖm vô ®i!", 0)
    end
end

function Task_TotalCancel()
    GiveItemUI(
        "Giao thñy tinh cho D· TÈu",
        "NÕu ng­¬i ®· quyÕt hñy bá chuçi nhiÖm vô, ta còng kh«ng ng¨n c¶n, h·y ®Æt vµo viªn Tö Thñy Tinh!",
...
function Task_ProcessInfo()
    ...
    Say(
        " HiÖn t¹i ng­êi ®· hoµn thµnh <color=yellow>" ..
            myCountTimes .. "<color> sè lÇn nhiÖm vô ta giao cho, cÇn cè g¾ng h¬n nhĞ!",
        0
    )
end
...
function Task_Accept()
    ...
    if (myTaskType == 1) then
        GiveItemUI(
            "NhiÖm vô t×m vËt phÈm",
            " ña? Nh÷ng thø ta cÇn ng­¬i t×m ®­îc cho ta ch­a?",
            "Task_Accept_01",
            "Task_Wait"
        )
    elseif (myTaskType == 2) then
        GiveItemUI(
            "NhiÖm vô mua vËt phÈm",
            " ña? Nh÷ng thø ta cÇn ng­¬i t×m ®­îc cho ta ch­a?",
            "Task_Accept_02",
            "Task_Wait"
        )
...
function Task_Accept_01(nCount)
    ...
    elseif (nCount == 0) then
        Say(
            " VŞ nµy" .. GetPlayerSex() .. ", ng­¬i cã thËt ®· bá vµo thø ta cÇn kh«ng? Kh«ng ph¶i l·o phu hoa m¾t chø?",
            0
        )
        return 0
    end
...
function Task_Accept_06()
    if (tl_checktask() == 1) then
        ...
        Say(
            " Hahaha! VŞ nµy" ..
                GetPlayerSex() ..
                    ", ta tuy bÊt tµi, nh÷ng còng hiÓu ®­îc ch÷ tİn trªn giang hå, ng­¬i cßn ch­a thu thËp ®ñ m¶nh s¬n Hµ X· T¾c mµ ta yªu cÇu sao cã thÓ ®Õn l·nh th­ëng ®©y?",
            0
        )
        return 0
    end
...
function Task_Cancel(nType)
    ...
    if (nType == 2) then
        if (myMapNum >= 100) then
            ...
            Say(
                " ng­¬i cã ®óng ®· mang <color=yellow>100<color> m¶nh s¬n Hµ X· T¾c kh«ng? Ta cã nh×n lÇm kh«ng vËy?",
                0
            )
            return
        end
...
function Task_Exit()
    if (GetSex() == 0) then
        Say(
            " Hahaha! §îi sau khi vŞ ®¹i hiÖp nµy th«ng qua kh¶o nghiÖm xong nhËn ®­îc c¸c phÇn th­ëng phong phó, tù nhiªn l¹i biÕt ®Õn t×m ta th«i!",
            0
        )
    else
        Say(
            " Hahaha! §îi sau khi vŞ n÷ hiÖp nµy th«ng qua kh¶o nghiÖm xong nhËn ®­îc c¸c phÇn th­ëng phong phó, tù nhiªn l¹i biÕt ®Õn t×m ta th«i!",
            0
        )
    end
end
...
function Task_Punish()
    Say(
        " VŞ nµy" ..
            GetPlayerSex() .. "Cã ph¶i gÊp l¾m kh«ng, kh¶o nghiÖm cña ta s¾p xÕp khã ®Õn nh­ vËy ­? LÇn sau ®Õn vËy!",
        0
    )
    return 0
end
...
function Task_GiveAward()
    ...
    Prise(
        "Ng­¬i vÊt v¶ qu¸, xin mêi vŞ " .. GetPlayerSex() .. " chän mãn m×nh thİch ®i! ",
        ShowText[1],
        ShowText[2],
        ShowText[3]
    )
    ...
    if (myQuality == 0) then
        ...
        if (GetProductRegion() == "vn") then
            ...
            if (myDetail == 238) or (myDetail == 239) or (myDetail == 240) or (myDetail == 252) then
                WriteLog(
                    " [Ghi nhí nhËn phÇn th­ëng]" ..
                        date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]") ..
                            ": Tµi kho¶n" ..
                                GetAccount() ..
                                    ", nh©n vËt" ..
                                        GetName() ..
                                            "Trong nhiÖm vô liªn tiÕp nhËn ®­îc phÇn th­ëng lµ b¶o th¹ch, sè thø tù lo¹i b¶o th¹ch lµ:" ..
                                                myDetail
                )
            elseif (myGenre == 6 and myDetail == 1 and myParticular == 1475) then
                ...
                WriteLog(
                    format(
                        "[Log phÇn th­ëng chuçi nhiÖm vô D· TÈu]%s: Tµi kho¶n %s, nh©n vËt %s hoµn thµnh chuçi nhiÖm vô D· TÈu nhËn ®­îc ®iÓm Tİch lòy D· TÈu (®iÓm kinh nghiÖm: %d)",
                        date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]"),
                        GetAccount(),
                        GetName(),
            ...
            WriteLog(
                " [Ghi nhí nhËn phÇn th­ëng]" ..
                    date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]") ..
                        " [mËt m·:" .. GetAccount() .. "] [nh©n vËt:" .. GetName() .. "]" .. "NhËn ®­îc 1 " .. szName
            )
        end
    else
        AddGoldItem(0, myGenre)
        AddGlobalNews(
            "Ng­êi ch¬i " .. GetName() .. " ®· hoµn thµnh nhiÖm vô D· TÈu nªn ®· nhËn ®­îc 1 trang bŞ Hoµng Kim!!!"
        )
        WriteLog(
            " [Ghi nhí nhËn phÇn th­ëng]" ..
                date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]") ..
                    ": Tµi kho¶n" ..
                        GetAccount() ..
                            ", nh©n vËt" ..
                                GetName() ..
                                    "Trong nhiÖm vô liªn tiÕp nhËn ®­îc phÇn th­ëng lµ trang bŞ Hoµng Kim sè thø tù lµ:" ..
                                        myGenre
        )
    end
...
function SelectAward_Money(nAward)
    ...
    if nAward1 >= 300000 then
        WriteLog(
            " [Ghi nhí chuçi nhiÖm vô]" ..
                date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]") ..
                    " [Tµi kho¶n]" ..
                        GetAccount() ..
                            " [Nh©n vËt]" ..
                                GetName() ..
                                    "Trong nhiÖm vô liªn tiÕp nhËn ®­îc<money>" ..
                                        nAward1 .. "</money>phÇn th­ëng lµ l­îng b¹c!"
        )
    end
    local nBeishu = greatnight_huang_event(4)
    local nTongValueGift = GetTaskTemp(TASKID_TONG_TASKLINKTEMP)
            ...
            date(" [%yn¨m%mth¸ng%dngµy%Hgiê%Mphót%Sgi©y]") ..
                " [Tµi kho¶n]" ..
                    GetAccount() ..
                        " [Nh©n vËt]" ..
                            GetName() ..
                                "B¹n nhËn ®­îc c¬ héi hñy bá nhiÖm vô, hiÖn t¹i cßn cã thÓ c¬ héi hñy <" ..
                                    myCancel .. "> lÇn."
    )
...
function PayPlayerLinkAward()
    ...
    -- local myGolden = {204, 205, 206, 207, 190, 191, 192, 193, 159, 160, 161, 162, 163, 164, 165, 166, 167} -- 20 »·½±Àø»Æ½ğ×°±¸µÄ±àºÅ
	...
	if nTotalTask == mocHTMoi100Lan  then
        tbAwardTemplet:GiveAwardByList(
            ...
            format("Get %s", "Tiªn Th¶o Lé")
        )
        Msg2Player("§· hoµn thµnh " .. nTotalTask .. " nhiÖm vô d· tÈu liªn tiÕp may m¾n nhËn ®­îc 3 Tiªn Th¶o Lé")
		mocHTMoi100Lan = SetTask(GetTask (5940),GetTask (5940)+100)
        return 
    end
    if nTotalTask == 100 then
        tbAwardTemplet:GiveAwardByList(
            {{szName = "B¶o R­¬ng §å Xanh ", tbProp = {6, 1, 4907, 1, 1}, nCount = 999}},  --> nCount = 999 nhËn 999 r­¬ng ®å xanh
            format("Get %s", "B¶o R­¬ng ®å xanh")
        )
        Msg2Player(
            "§· hoµn thµnh " .. nTotalTask .. " nhiÖm vô d· tÈu liªn tiÕp may m¾n nhËn ®­îc 999 B¶o R­¬ng §å Xanh"
        )
        return
    end
    ...
    if nTotalTask == 200 then
        tbAwardTemplet:GiveAwardByList(
            {{szName = "B¶o R­¬ng §å Xanh", tbProp = {6, 1, 4907, 1, 1}, nCount = 999}},
            format("Get %s", "B¶o R­¬ng §å Xanh")
        )
        Msg2Player(
            "§· hoµn thµnh " .. nTotalTask .. " nhiÖm vô d· tÈu liªn tiÕp may m¾n nhËn ®­îc 999 B¶o R­¬ng §å Xanh"
    ...
    if nTotalTask == 1000 then
        tbAwardTemplet:GiveAwardByList(
            {{szName = "B¶o R­¬ng §éng S¸t", tbProp = {6, 1, 4910, 1, 1}, nCount = 1}},
            format("Get %s", "B¶o R­¬ng §éng S¸t")
        )
        Msg2Player(
            "§· hoµn thµnh " .. nTotalTask .. " nhiÖm vô d· tÈu liªn tiÕp may m¾n nhËn ®­îc 1 B¶o R­¬ng §éng S¸t"
        )
        return
    end
    if nTotalTask == 2000 then
        tbAwardTemplet:GiveAwardByList(
            {{szName = "B¶o R­¬ng An Bang TiÓu", tbProp = {6, 1, 4909, 1, 1}, nCount = 1}},
            format("Get %s", "B¶o R­¬ng An Bang TiÓu")
        )
        Msg2Player(
            "§· hoµn thµnh " .. nTotalTask .. " nhiÖm vô d· tÈu liªn tiÕp may m¾n nhËn ®­îc 1 B¶o R­¬ng An Bang TiÓu"
        )
        return
    end
    if nTotalTask == 3000 then
        tbAwardTemplet:GiveAwardByList(
            ...
            format("Get %s", "An Bang B¨ng Tinh Th¹ch H¹ng Liªn")
        )
        Msg2Player(
            "§· hoµn thµnh " ..
                nTotalTask .. " nhiÖm vô d· tÈu liªn tiÕp may m¾n nhËn ®­îc 1 An Bang B¨ng Tinh Th¹ch H¹ng Liªn"
        )
        return
    end

    -- if nTotalTask == 2500 or nTotalTask == 4500 then
    --tbAwardTemplet:GiveAwardByList({{szName = "Trang BŞ HKMP", tbProp ={ 0,random(1,140)},nCount=1,nQuality=1}}, format("Get %s", "An Bang B¨ng Tinh Th¹ch H¹ng Liªn"))
    --	Msg2Player("§· hoµn thµnh "..nTotalTask.." nhiÖm vô d· tÈu liªn tiÕp may m¾n nhËn ®­îc 1 Trang BŞ HKMP");
    --	return
    -- end
    ...
    --tbAwardTemplet:GiveAwardByList({{szName = "Trang BŞ Vò Khİ HKMP", tbProp ={ 0,idvatpham[henxui]},nCount=1,nQuality=1}}, format("Get %s", "An Bang B¨ng Tinh Th¹ch H¹ng Liªn"))
    --	Msg2Player("§· hoµn thµnh "..nTotalTask.." nhiÖm vô d· tÈu liªn tiÕp may m¾n nhËn ®­îc 1 Trang BŞ Vò Khİ HKMP");
    --	return
    -- end
    -- if nTotalTask == 9000 then
    --tbAwardTemplet:GiveAwardByList({{szName = "V« Danh ChØ Hoµn", tbProp ={ 0,141},nCount=1,nQuality=1}}, format("Get %s", "An Bang B¨ng Tinh Th¹ch H¹ng Liªn"))
    --	Msg2Player("§· hoµn thµnh "..nTotalTask.." nhiÖm vô d· tÈu liªn tiÕp may m¾n nhËn ®­îc 1 V« Danh ChØ Hoµn");
    --	return
    -- end
    -- if nTotalTask == 12000 then
    --tbAwardTemplet:GiveAwardByList({{szName = "V« Danh Giíi ChØ", tbProp ={ 0,142},nCount=1,nQuality=1}}, format("Get %s", "An Bang B¨ng Tinh Th¹ch H¹ng Liªn"))
    --	Msg2Player("§· hoµn thµnh "..nTotalTask.." nhiÖm vô d· tÈu liªn tiÕp may m¾n nhËn ®­îc 1 V« Danh Giíi ChØ");
    --	return
    -- end
    -- if nTotalTask == 500 then
    --tbAwardTemplet:GiveAwardByList({{szName = "Trang bŞ §Şnh Quèc", tbProp ={ 0,random(159,163)},nQuality=1, nCount=1}}, format("Get %s", "Trang BŞ §Şnh Quèc"))
    --	Msg2Player("§· hoµn thµnh "..nTotalTask.." nhiÖm vô d· tÈu liªn tiÕp may m¾n nhËn ®­îc 1 Trang BŞ §Şnh Quèc");
    --	return
    -- end
...
function _CancelTaskDebug()
    ...
    if nOrgTask == nDecTask or nOrgTask == 0 then
        ...
        WriteLog(
            " [B¶ng nhiÖm vô ®· ghi nhí sai]" ..
                date(" [%yn¨m%mth¸ng%dngµy%Hgiê%Mphót%Sgi©y]") ..
                    " [Tµi kho¶n]" ..
                        GetAccount() ..
                            " [Nh©n vËt]" ..
                                GetName() ..
                                    "C¬ héi hñy bá kh«ng c©n b»ng, hiÖn t¹i c¬ héi hñy bá cßn <" ..
                                        nDecTask .. "> lÇn, phÇn c¬ héi hñy bá cßn <" .. nOrgTask .. ">."
        )
        return 0
    end
end
...
function _WriteCancelLog(nType, nTime, nCancel)
    if nType == 1 then
        WriteLog(
            " [Hñy bá kû lôc nhiÖm vô liªn tiÕp]" ..
                date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]") ..
                    ": Tµi kho¶n" ..
                        GetAccount() ..
                            ", nh©n vËt" ..
                                GetName() ..
                                    "T¹i nhiÖm vô thø " ..
                                        nTime .. "Hñy bá c¬ héi cßn d­ " .. nCancel .. "giê hñy bá mét lÇn nhiÖm vô."
        )
    else
        WriteLog(
            " [Hñy bá kû lôc nhiÖm vô liªn tiÕp]" ..
                date(" [%y n¨m %m th¸ng %d ngµy  %H giê %M phót]") ..
                    ": Tµi kho¶n" ..
                        GetAccount() ..
                            ", nh©n vËt" ..
                                GetName() ..
                                    "T¹i nhiÖm vô thø " .. nTime .. " dïng 100 tÊm s¬n Hµ X· T¾c hñy bá nhiÖm vô."
        )
    end
end
...
function menglanjie()
    ...
    Say(
        "ChØ cÇn ng­¬i ®i thu thËp ®ñ sè Liªn Hoa vµ <color=yellow>" ..
            MAKING_COST ..
                "<color> l­îng ta sÏ gióp ng­¬i kÕt vßng hoa tuyÖt ®Ñp. Nh­ng l­u ı: mçi ngµy chØ cã thÓ kÕt ®­îc 2 vßng Liªn Hoa cïng thuéc tİnh mµ th«i.",
        getn(tab_Content),
        tab_Content
    )
end
...
tab_Flower = {
    ...
    {1127, "Méc", 1132, 1762, 1763},
    {1128, "Thñy ", 1133, 1764, 1765},
    {1129, "Háa", 1134, 1766, 1767},
    {1130, "Thæ ", 1135, 1768, 1769}
}
...
function process(nIdx)
    ...
    Say(
        "Muèn lµm 1 vßng <color=yellow>" ..
            tab_Flower[nIdx][2] ..
                " Liªn Hoa<color> cÇn cã 9 <color=yellow>" ..
                    tab_Flower[nIdx][2] .. "Liªn Hoa<color>. Ng­¬i x¸c ®Şnh lµm chø?",
        2,
        "§óng! Xin l·o bèi træ tµi!/#make_round(" .. nIndex .. ")",
        "Ta sÏ quay l¹i sau!/Task_Wait"
    )
end
...
function make_round(nIdx)
    ...
    if (nCount < 9) then
        Say(
            "Ng­¬i h×nh nh­ ch­a ®ñ <color=yellow>" ..
                tab_Flower[nIdx][2] ..
                    " Liªn Hoa<color=yellow>. Ch­a ®ñ 9 <color=yellow>" ..
                        tab_Flower[nIdx][2] ..
                            " Liªn Hoa<color> th× ta kh«ng thÓ gióp ng­¬i kÕt vßng " ..
                                tab_Flower[nIdx][2] .. " Liªn Hoa hoµn",
            1,
            "§Ó ta ®i chuÈn bŞ ®·!/Task_Wait"
        )
        return
    end
    ...
    Say(
        "Vßng " .. tab_Flower[nIdx][2] .. " Liªn Hoa cña ng­¬i ®· kÕt xong. H·y mang nã ®Õn chç LÔ Quan ®i!",
        1,
        "NhËn vßng hoa/Task_Wait"
    )
